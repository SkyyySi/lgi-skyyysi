...
function testsuite.group.new(name)
	local group = setmetatable({ name = name, results = { total = 0, failed = 0 } }, testsuite.group)
...
function testsuite.group:run(id)
	...
	if type(id) == "number" then
		...
		for i = 1, #self do
			if self[i] ~= "debug" and self[i]:match(id) then
				runfunc(i)
			end
		end
		if self.results.failed == 0 then
			io.write(("%-8s: all %d tests passed.\n"):format(self.name, self.results.total))
		else
			io.write(("%-8s: FAILED %d of %d tests\n"):format(self.name, self.results.failed, self.results.total))
		end
	end
end
...
function testsuite.check(cond, msg, skip)
	if not cond then
		testsuite.fail(msg, (skip or 1) + 1)
	end
end
...
function testsuite.checkv(val, exp, exptype)
	if exptype then
		testsuite.check(type(val) == exptype, string.format("got type `%s', expected `%s'", type(val), exptype), 2)
	end
	testsuite.check(val == exp, string.format("got value `%s', expected `%s'", tostring(val), tostring(exp)), 2)
end
