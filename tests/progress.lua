...
local function check_gerror(namespace, api, ...)
	...
	check(result, string.format("Error during %s() call: %s", api, tostring(err)))
	return result
end
...
function progress.file_copy()
	...
	local progress_callback = function(partial, total)
		check(partial <= total, string.format("Writing too many bytes (%d > %d)", partial, total))
	end
	...
	src:copy_async(dst, flags, priority, cancellable, progress_callback, finish_callback)
	loop:run()
end
