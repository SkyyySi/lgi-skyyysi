...
function cairo.status()
	...
	for name, value in pairs(cairo.Status) do
		if type(name) == "string" and type(value) == "number" then
			checkv(cairo.Status.to_string(name), cairo.Status.to_string(value), "string")
		end
	end
end
...
function cairo.matrix()
	...
	matrix = cairo.Matrix { xx = 1, yx = 1.5, xy = 2, yy = 2.5, x0 = 3, y0 = 3.5 }
...
function cairo.matrix_getset()
	...
	local m = cairo.Matrix { xx = 1, yx = 1.5, xy = 2, yy = 2.5, x0 = 3, y0 = 3.5 }
	cr.matrix = m
	local m2 = cr.matrix
	check(m.xx == m2.xx)
	check(m.yx == m2.yx)
