...
local function checkvf(val, exp, tolerance)
	check(type(val) == "number", string.format("got type `%s', expected `number'", type(val)), 2)
	check(math.abs(val - exp) <= tolerance, string.format("got value `%s', expected `%s'", tostring(val), tostring(exp)), 2)
end
...
function gireg.ghash_nested_everything_return()
	...
	check(a.wibble.foo == "bar" and a.wibble.baz == "bat" and a.wibble.qux == "quux")
end
...
function gireg.struct_a()
	...
	a = R.TestStructA { some_int = 42, some_int8 = 12, some_double = 3.14, some_enum = "VALUE2" }
	a.some_int = 43
	a.some_int8 = 13
	check(a.some_int == 43)
...
function gireg.struct_a_clone()
	local R = lgi.Regress
	local a = R.TestStructA { some_int = 42, some_int8 = 12, some_double = 3.14, some_enum = R.TestEnum.VALUE2 }
	check(a == a)
...
function gireg.struct_b()
	...
	b.nested_a = { some_int = 42, some_int8 = 12, some_double = 3.14, some_enum = R.TestEnum.VALUE2 }
	...
	b = R.TestStructB { some_int8 = 21, nested_a = { some_int = 42, some_int8 = 12, some_double = 3.14, some_enum = R.TestEnum.VALUE2 } }
...
function gireg.struct_b_clone()
	local R = lgi.Regress
	local b = R.TestStructB { some_int8 = 21, nested_a = { some_int = 42, some_int8 = 12, some_double = 3.14, some_enum = R.TestEnum.VALUE2 } }
	check(b == b)
...
function gireg.boxed_a_equals()
	local R = lgi.Regress
	check(R.TestSimpleBoxedA({ some_int = 1, some_int8 = 2, some_double = 3.14 }):equals(R.TestSimpleBoxedA { some_int = 1, some_int8 = 2, some_double = 3.14 }))
	check(not R.TestSimpleBoxedA({ some_int = 2, some_int8 = 2, some_double = 3.14 }):equals(R.TestSimpleBoxedA { some_int = 1, some_int8 = 2, some_double = 3.14 }))
...
function gireg.obj_prop_hash()
	...
	check(not pcall(function()
		o.hash_table = function() end
	end))
	check(not pcall(function()
		o.hash_table = R.TestObj()
	end))
	check(not pcall(function()
		o.hash_table = R.TestBoxed()
	end))
end
...
function gireg.callback_simple()
	local R = lgi.Regress
	check(R.test_callback(function()
		return 42
	end) == 42)
	...
	check(R.test_multi_callback(function()
		return 22
	end) == 44)
...
function gireg.callback_data()
	local R = lgi.Regress
	local called
	R.test_simple_callback(function()
		called = true
	end)
	check(called)
	check(R.test_callback_user_data(function()
		return 42
	end) == 42)

	called = nil
	R.TestObj.static_method_callback(function()
		called = true
		return 42
	end)
	check(called)
	local o = R.TestObj()
	called = nil
	o.static_method_callback(function()
		called = true
		return 42
	end)
	check(called)
	called = nil
	o:instance_method_callback(function()
		called = true
		return 42
	end)
	check(called)
...
function gireg.callback_notified()
	local R = lgi.Regress
	check(R.test_callback_destroy_notify(function()
		return 1
	end) == 1)
	check(R.test_callback_destroy_notify(function()
		return 2
	...
	R.TestObj.new_callback(function()
		return 1
	end)
	collectgarbage()
	collectgarbage()
...
function gireg.callback_async()
	local R = lgi.Regress
	R.test_callback_async(function()
		return 1
	end)
	collectgarbage()
	collectgarbage()
