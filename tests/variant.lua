...
function variant.gvalue()
	local var1, var2 = GLib.Variant.new_string("foo"), GLib.Variant.new_string("bar")
...
function variant.newv_maybe()
	local V, v = GLib.Variant
	v = V("mi", 42)
	check(v.type == "mi" and v:n_children() == 1 and v:get_child_value(0).type == "i" and v:get_child_value(0):get_int32() == 42)
	v = V("mi")
...
function variant.newv_tuple()
	...
	v = V.new("(i)", { 42 })
	check(v.type == "(i)" and v:n_children() == 1 and v:get_child_value(0).type == "i" and v:get_child_value(0):get_int32() == 42)
	v = V.new("(mii)", { nil, 1 })
	check(v.type == "(mii)" and v:n_children() == 2 and v:get_child_value(0):n_children() == 0)
end

function variant.newv_dictentry()
	local V, v = GLib.Variant
	v = V("{is}", { 42, "Hello" })
	check(v.type == "{is}" and v:n_children() == 2 and v:get_child_value(0).type == "i" and v:get_child_value(0):get_int32() == 42 and v:get_child_value(1).type == "s" and v:get_child_value(1):get_string() == "Hello")
end

function variant.newv_array()
	local V, v = GLib.Variant
	v = V("as", { "Hello", "world" })
	check(v.type == "as" and v:n_children() == 2 and v:get_child_value(0):get_string() == "Hello" and v:get_child_value(1):get_string() == "world")
	v = V("as", {})
	check(v:n_children() == 0)
	v = V("ams", { "Hello", nil, "world", n = 3 })
	check(v:n_children() == 3)
	check(v:get_child_value(0):n_children() == 1 and v:get_child_value(0):get_child_value(0):get_string() == "Hello")
	check(v:get_child_value(1):n_children() == 0)
	check(v:get_child_value(2):n_children() == 1 and v:get_child_value(2):get_child_value(0):get_string() == "world")
end
...
function variant.value_container()
	...
	check(type(r) == "table" and r.n == 3 and r[1] == 2 and r[2] == nil and r[3] == 1)
	v = V("as", {})
	check(v.value == v)
end
