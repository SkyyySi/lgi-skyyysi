...
local pairs, ipairs, setmetatable, getmetatable = pairs, ipairs, setmetatable, getmetatable
...
function ffi.load_gtype(resolver, get_type_name)
	...
	if not addr then
		return nil, ("`%s' not found"):format(get_type_name)
	end
	local get_gtype = core.callable.new { name = get_type_name, addr = addr, ret = ffi.types.GType }
	return get_gtype()
end
...
function ffi.load_fields(rec, defs)
	...
	rec._size = ((getmetatable(rec)._type == "union") and max_size or align(offset, max_align))
end
