...
#define g_atomic_int_add(atomic, val) (g_atomic_int_exchange_and_add((atomic), (val)))
#endif
...
void *lgi_udata_test(lua_State *L, int narg, const char *name) {
	void *udata = NULL;
	luaL_checkstack(L, 2, "");
	lgi_makeabs(L, narg);
...
void lgi_cache_create(lua_State *L, gpointer key, const char *mode) {
...
GType lgi_type_get_gtype(lua_State *L, int narg) {
	...
	default:
		return luaL_error(L, "GType expected, got %s", lua_typename(L, lua_type(L, narg)));
	}
}
...
static int core_constant(lua_State *L) {
	...
	lgi_marshal_2lua(L, ti, NULL, GI_DIRECTION_IN, GI_TRANSFER_NOTHING, &val, 0, NULL, NULL);
	return 1;
}
...
static int call_mutex_gc(lua_State *L) {
...
void lgi_state_enter(gpointer state_lock) {
...
static const char *log_levels[] = {"ERROR", "CRITICAL", "WARNING", "MESSAGE", "INFO", "DEBUG", "???", NULL};

static int core_log(lua_State *L) {
...
static void package_lock_enter(void) {
	g_rec_mutex_lock(&package_mutex);
}

static void package_lock_leave(void) {
	g_rec_mutex_unlock(&package_mutex);
}

static gpointer package_lock_register[8] = {NULL};

static int core_registerlock(lua_State *L) {
	...
	for (i = 0; i < G_N_ELEMENTS(package_lock_register) && package_lock_register[i] != set_lock_functions; i++) {
...
static int core_band(lua_State *L) {
	lua_pushinteger(L, (lgi_Unsigned)luaL_checkinteger(L, 1) & (lgi_Unsigned)luaL_checkinteger(L, 2));
	return 1;
}

static int core_bor(lua_State *L) {
	lua_pushinteger(L, (lgi_Unsigned)luaL_checkinteger(L, 1) | (lgi_Unsigned)luaL_checkinteger(L, 2));
	return 1;
}
...
static const struct luaL_Reg module_reg[] = {{"__gc", module_gc}, {"__index", module_index}, {NULL, NULL}};
...
static int core_module(lua_State *L) {
	char *name;
	...
	if (!lua_isnoneornil(L, 2)) {
		name = g_strdup_printf(MODULE_NAME_FORMAT_VERSION, luaL_checkstring(L, 1), (int)luaL_checkinteger(L, 2));
	} else
#endif
		name = g_strdup_printf(MODULE_NAME_FORMAT_PLAIN, luaL_checkstring(L, 1));

#if defined(__APPLE__)
	char *path = g_module_build_path(GOBJECT_INTROSPECTION_LIBDIR, name);
	g_free(name);
	name = path;
#endif
...
static const struct luaL_Reg lgi_reg[] = {{"log", core_log}, {"gtype", core_gtype}, {"repotype", core_repotype}, {"constant", core_constant}, {"yield", core_yield}, {"registerlock", core_registerlock}, {"band", core_band}, {"bor", core_bor}, {"module", core_module}, {"upcase", core_upcase}, {"downcase", core_downcase}, {NULL, NULL}};

static void create_repo_table(lua_State *L, const char *name, void *key) {
	lua_newtable(L);
	lua_pushlightuserdata(L, key);
	lua_pushvalue(L, -2);
	lua_rawset(L, LUA_REGISTRYINDEX);
...
static void set_resident(lua_State *L) {
	   ...
	   passed as arg 2. */
		...
		if (lua_gettop(L) == 3) {
			/* Some Lua versions give us the path to the .so on the stack.
			   Just load & leak it. */
			GModule *module = g_module_open(lua_tostring(L, 2), G_MODULE_BIND_LAZY | G_MODULE_BIND_LOCAL);
			if (module != NULL) {
				return;
			}
		}
	   ...
	   closed. */
		...
		while (lua_next(L, LUA_REGISTRYINDEX)) {
			if (lua_type(L, -2) == LUA_TSTRING) {
				...
				if (g_str_has_prefix(str, "LOADLIB: ") && strstr(str, "corelgilua5")) {
...
G_MODULE_EXPORT int luaopen_lgi_corelgilua51(lua_State *L) {
	LgiStateMutex *mutex;
	gint           state_id;
