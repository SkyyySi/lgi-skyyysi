...
local pairs, ipairs, setmetatable, select = pairs, ipairs, setmetatable, select
...
for name, def in pairs {
	...
	MarkupParser._attribute[name] = {
		set = function(parser, target)
...
function MarkupParseContext.new(parser, flags, user_data)
	...
	-- provide dummy one.
	return MarkupParseContext._method.new(parser, flags, user_data, function() end)
end
