...
for k, v in pairs {
	...
	STRING_ARRAY = "as",
	BYTESTRING = "ay",
	BYTESTRING_ARRAY = "aay",
} do
	VariantType[k] = VariantType.new(v)
end
...
local function read_format(format, pos, basic)
	...
	elseif t == "{" then
		...
		if not pos or format:sub(pos, pos) ~= "}" then
			return nil
		end
		return pos + 1
	elseif t == "(" then
...
function variant_new(format, pos, val)
	...
	elseif t == "m" then
		...
		return Variant.new_maybe(VariantType.new(format:sub(pos, epos - 1)), variant), epos
	elseif t == "a" then
		...
		if et:is_subtype_of(VariantType.DICT_ENTRY) then
			...
			for k, v in pairs(val) do
				builder:add_value(Variant.new_dict_entry(variant_new(format, pos + 1, k), variant_new(format, pos + 2, v)))
			end
		else
	...
	elseif t == "(" or t == "{" then
		...
		local builder = VariantBuilder.new(VariantType.new(format:sub(pos - 1, epos - 1)))
...
function Variant:_access_index(variant, index, ...)
	...
	if Variant.is_container(variant) and Variant.n_children(variant) >= index then
