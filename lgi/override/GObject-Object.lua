...
local pairs, select, setmetatable, error, type = pairs, select, setmetatable, error, type
...
function Object:_construct(gtype, param, owns)
	...
	if not Type.is_a(gtype, self._gtype) then
		error(("`%s' is not subtype of `%s'"):format(Type.name(gtype), self._name), 3)
	end
...
local get_property_guard, get_property_addr = core.marshal.callback(gi.GObject.ObjectClass.fields.get_property.typeinfo.interface, function(self, prop_id, value, pspec)
...
local set_property_guard, set_property_addr = core.marshal.callback(gi.GObject.ObjectClass.fields.get_property.typeinfo.interface, function(self, prop_id, value, pspec)
	local name = pspec.name:gsub("%-", "_")
...
function Object:_element(object, name)
	...
	for i = 1, #interfaces do
		...
		if iface then
			element, category = iface:_element(object, name, self)
		end
		if element then
			return element, category
		end
	...
	local property = Object._class.find_property(object._class, name:gsub("_", "-"))
	if property then
...
local function marshal_property(obj, name, flags, gtype, marshaller, ...)
	...
	if not flags[mode] then
		error(("%s: `%s' not %s"):format(core.object.query(obj, "repo")._name, name, core.downcase(mode)))
	end
...
function Object:_access_property(object, prop, ...)
	if gi.isinfo(prop) then
		...
		local marshaller = Value.find_marshaller(gtype, typeinfo, prop.transfer)
		return marshal_property(object, prop.name, repo.GObject.ParamFlags[prop.flags], gtype, marshaller, ...)
	else
		-- pspec-based property
		return marshal_property(object, prop.name, prop.flags, prop.value_type, Value.find_marshaller(prop.value_type), ...)
	end
end
...
local function connect_signal(obj, gtype, name, closure, detail, after)
	return signal_connect_closure_by_id(obj, signal_lookup(name, gtype), detail and quark_from_string(detail) or 0, closure, after or false)
end
...
local function emit_signal(obj, gtype, info, detail, ...)
	...
	-- Invoke the signal.
	signal_emitv(params, signal_lookup(info.name, gtype), detail and quark_from_string(detail) or 0, retval)
...
function Object:_access_signal(object, info, ...)
	...
	if select("#", ...) > 0 then
		...
		function pad:connect(target, detail, after)
			return connect_signal(object, gtype, info.name, Closure(target, info), detail, after)
		end
		function pad:emit(...)
		...
		if not info.is_signal or info.flags.detailed then
			...
			function mt:__newindex(detail, target)
				connect_signal(object, gtype, info.name, Closure(target, info), detail)
			end
		end
