...
if Gdk.Atom then
	...
	for name, val in pairs {
		...
		Gdk._constant[name] = Gdk.Atom(val)
	end
end
...
for _, name in pairs { "get_clip_rectangle", "set_source_color", "set_source_pixbuf", "set_source_window", "region" } do
	cairo.Context._method[name] = Gdk["cairo_" .. name]
end
for _, name in pairs { "clip_rectangle", "source_color", "source_pixbuf", "source_window" } do
