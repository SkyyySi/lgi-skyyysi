...
local assert, pairs, select, type, tostring, error = assert, pairs, select, type, tostring, error
...
for _, name in pairs { "set_object", "set_variant", "set_string" } do
	if not gi.GObject.Value.methods[name].args[1].optional then
		...
		Value._method[name] = function(value, val)
			if not val then
				Value.reset(value)
			else
				setter(value, val)
	...
	if gtype then
		if newtype then
			...
			if not Value.transform(value, dest) then
				error(("GObject.Value: cannot convert `%s' to `%s'"):format(gtype, core.record.field(dest, value_field_gtype)))
			end
			Value.unset(value)
...
value_marshallers[Type.BOXED] = function(value, params, ...)
...
-- g_shell_parse_argv().
value_marshallers[Type.STRV] = core.marshal.container(gi.GLib.shell_parse_argv.args[3].typeinfo)
...
function Value._attribute:value(...)
	local marshaller = Value._method.find_marshaller(core.record.field(self, value_field_gtype))
