...
local Type = { STRV = "GStrv", ARRAY = "GArray", BYTE_ARRAY = "GByteArray", PTR_ARRAY = "GPtrArray", HASH_TABLE = "GHashTable", ERROR = "GError", GTYPE = "GType" }
repo.GObject._struct = { Type = Type }
for _, name in pairs { "name", "qname", "from_name", "parent", "depth", "next_base", "is_a", "children", "interfaces", "query", "fundamental_next", "fundamental" } do
	Type[name] = repo.GObject["type_" .. name]
end
for num, name in ipairs { "NONE", "INTERFACE", "CHAR", "UCHAR", "BOOLEAN", "INT", "UINT", "LONG", "ULONG", "INT64", "UINT64", "ENUM", "FLAGS", "FLOAT", "DOUBLE", "STRING", "POINTER", "BOXED", "PARAM", "OBJECT", "VARIANT" } do
	...
	if not gtype then
		...
		elseif ti.tag == "array" then
			...
			if atype == "c" then
				...
				if (etag == "utf8" or etag == "filename") and ti.is_zero_terminated then
					gtype = Type.STRV
				end
			else
				gtype = ({ array = Type.ARRAY, byte_array = Type.BYTE_ARRAY, ptr_array = Type.PTR_ARRAY })[atype]
			end
		end
	end
	return gtype
