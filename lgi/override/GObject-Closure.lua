...
function CallInfo.new(callable_info, to_lua)
	local self = setmetatable({ has_self = (callable_info.is_signal or callable_info.is_virtual) }, CallInfo)
	local argc, gtype = 0
	...
	local function mark_array_length(cell, ti)
		...
		if len then
			...
			if not self[cell.len_index] then
				self[cell.len_index] = {}
			end
	...
	if self.has_self then
		...
		self[1] = { dir = "in", gtype = gtype, [to_lua and "to_lua" or "to_value"] = Value.find_marshaller(gtype) }
	end
	...
	if ti.tag ~= "void" or ti.is_pointer then
		...
		local ret = { dir = "out", gtype = gtype, to_value = Value.find_marshaller(gtype, ti, callable_info.return_transfer) }
...
local function marshal_cell(call_info, cell, direction, args, argc, marshalling_params, value, params, retval)
	local marshaller = cell[direction]
	if not marshaller or cell.internal then
		return argc
	end
	argc = argc + 1
	...
	if cell.len_index then
		...
		if direction == "to_lua" then
			marshalling_params.length = length_marshaller(params[cell.len_index], {})
		end
	end
	if direction == "to_lua" then
		...
		if length_marshaller then
			length_marshaller(params[cell.len_index], {}, marshalling_params.length)
		end
...
function CallInfo:get_closure_marshaller(target)
	return function(closure, retval, params)
		...
		for i = 1, #self do
			argc = marshal_cell(self, self[i], "to_lua", args, argc, marshalling_params, params[i], params)
		end
		...
		if self.ret and retval then
			argc = marshal_cell(self, self.ret, "to_value", args, argc, marshalling_params, retval, params)
		end
		...
		for i = 1, #self do
			argc = marshal_cell(self, self[i], "to_value", args, argc, marshalling_params, params[i], params, retval)
		end
	end
end
...
function CallInfo:pre_call(...)
	...
	local params = {}
	for i = 1, #self do
		params[#params + 1] = Value(self[i].gtype)
	end
	local marshalling_params = { keepalive = {} }
	...
	local args, argc = { ... }, 0
	for i = 1, #self do
		argc = marshal_cell(self, self[i], "to_value", args, argc, marshalling_params, params[i], params)
	end

	-- Prepare return value.
	local retval = Value()
	if self.ret then
		retval.type = self.ret.gtype
	end
	if self.phantom then
		retval.type = self.phantom.gtype
	end
...
function CallInfo:post_call(params, retval, marshalling_params)
	...
	-- yes, return just nil.
	if self.phantom and not self.phantom.to_lua(retval, marshalling_params) then
		return nil
	end

	-- Unmarshal return value.
	if self.ret and retval then
		argc = marshal_cell(self, self.ret, "to_lua", args, argc, marshalling_params, retval, params)
	end
