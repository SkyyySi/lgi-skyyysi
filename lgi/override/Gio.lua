...
local select, type, pairs, setmetatable, rawset, unpack = select, type, pairs, setmetatable, rawset, unpack or table.unpack
...
local function async_element(name, accessor, param_self, param_object)
	...
	if name_root then
		...
		if finish and not async and accessor(param_self, param_object, name_root .. "_sync") then
		...
		if async and finish then
			element = { name = name_root, in_args = 0, async = async, finish = finish }
			...
			for _, param in ipairs(async.params) do
				if param["in"] then
					...
					if not param["out"] and param.typeinfo then
						if param.name == "io_priority" and param.typeinfo.tag == tag_int then
							element.io_priority = element.in_args
						elseif param.name == "cancellable" and param.typeinfo.tag == "interface" then
...
local function async_access(element, process_yield)
	...
	return function(...)
		...
		if not context then
			error(("%s.async_%s: called out of async context"):format(self._name, element.name), 4)
		end
...
function class.class_mt:_index_async(element)
	return async_access(element, function(_, ...)
		return ...
	end)
end
...
function GObject.Object:_access_async(object, element, ...)
	if select("#", ...) > 0 then
		error(("%s: `%s' not writable"):format(core.object.query(object, "repo")._name, name))
	end

	return async_access(element, function(...)
		return ...
	end)
end
...
function namespace.mt._category_mt._function:__index(key)
	...
	return element and async_access(element, function(_, ...)
		return ...
	end)
end
...
function Gio.AsyncInitable._init1(object)
	...
	if not async_context[coroutine.running()] then
		return "_initskip"
	end
	...
	if gi.Gio.InputStream.methods[name].args[1].typeinfo.tag ~= "array" then
		...
		if name == "read_async" then
			...
			function Gio.InputStream.async_read(stream, buffer)
				raw_read(stream, buffer, Gio.Async.io_priority, Gio.Async.cancellable, coroutine.running())
