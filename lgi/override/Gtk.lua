...
local select, type, pairs, ipairs, unpack, setmetatable, error, next, rawget = select, type, pairs, ipairs, unpack or table.unpack, setmetatable, error, next, rawget
...
for _, name in pairs { "allocation", "direction", "settings", "realized", "mapped", "display", "screen", "window", "root_window", "has_window", "style_context" } do
	if not Gtk.Widget._property[name] then
		local attr = { get = Gtk.Widget["get_" .. name], set = Gtk.Widget["set_" .. name] }
		if next(attr) then
...
if core.gi.Gtk.Widget.methods.intersect and core.gi.Gtk.Widget.methods.intersect.args[2].direction == "in" then
...
function widget_style_mt:__index(name)
	...
	if not pspec then
		error(("%s: no style property `%s'"):format(self._widget.type._name, name:gsub("%-", "_")), 2)
	end
...
if Gtk.Container then
	...
	function container_property_item_mt:__index(name)
		...
		if not pspec then
			error(("%s: no child property `%s'"):format(self._container.type._name, name:gsub("%-", "_")), 2)
		end
		local value = GObject.Value(pspec.value_type)
	...
	function container_property_mt:__index(child)
		if type(child) == "string" then
			child = self._container.child[child]
		end
		return setmetatable({ _container = self._container, _child = child }, container_property_item_mt)
	end
	...
	function container_child_mt:__index(id)
		...
		local found = (core.object.env(self._container).id == id and self._container)
		if not found then
...
function Gtk.Builder:add_objects_from_file(filename, object_ids)
	return builder_fix_return(Gtk.Builder._method.add_objects_from_file(self, filename, object_ids))
end
function Gtk.Builder:add_from_string(string, len)
	if not len or len == -1 then
		len = #string
	end
	return builder_fix_return(Gtk.Builder._method.add_from_string(self, string, len))
end
function Gtk.Builder:add_objects_from_string(string, len, object_ids)
	if not len or len == -1 then
		len = #string
	end
	return builder_fix_return(Gtk.Builder._method.add_objects_from_string(self, string, len, object_ids))
end
...
function Gtk.Builder._method:connect_signals(handlers)
	local unconnected
	self:connect_signals_full(function(builder, object, signal, handler, connect_object, flags)
		signal = "on_" .. signal:gsub("%-", "_")
		local target = handlers[handler]
		if not target then
			unconnected = unconnected or {}
			unconnected[#unconnected + 1] = handler
			log.warning("%s: failed to connect to `%s' handler", signal, handler)
			return
		end
		local fun
		if connect_object then
			fun = function(_, ...)
				return target(connect_object, ...)
			end
		else
			fun = target
		end
...
function Gtk.TreeModel:set(iter, values)
	...
	if Gtk.TreePath:is_type_of(iter) then
		iter = self:get_iter(iter)
	end
...
function Gtk.TreeModel:next(iter)
	if not iter or type(iter) == "table" then
		...
		if not self:iter_next(iter) then
			iter = nil
		end
	end
...
function Gtk.ListStore:insert(position, values)
	...
	if not values then
		...
		iter = Gtk.ListStore._method.insert_with_valuesv(self, position, treemodel_prepare_values(self, values))
	end
	return iter
end
if not Gtk.ListStore._method.insert_with_values then
	Gtk.ListStore._method.insert_with_values = Gtk.ListStore._method.insert_with_valuesv
end
function Gtk.ListStore:append(values)
	...
	if not values then
		...
		iter = Gtk.ListStore._method.insert_with_values(self, -1, treemodel_prepare_values(self, values))
	end
	return iter
end
...
function Gtk.TreeStore:insert(parent, position, values)
	...
	if not values then
		...
		iter = Gtk.TreeStore._method.insert_with_values(self, parent, position, treemodel_prepare_values(self, values))
	end
	return iter
end
function Gtk.TreeStore:append(parent, values)
	...
	if not values then
		...
		iter = Gtk.TreeStore._method.insert_with_values(self, parent, -1, treemodel_prepare_values(self, values))
	end
	return iter
end
...
function Gtk.CellLayout:add(def)
	...
	if def.data_func then
		self:set_cell_data_func(def[1], def.data_func)
	end
end
	...
	function Gtk.ActionGroup:add(action)
		if type(action) == "table" then
			...
			if first_radio and action.on_change then
				...
				function first_radio:on_changed(current)
					on_change(current)
				end
			end
		else
...
function assistant_property_mt:__newindex(property_name, value)
	self._assistant["set_page_" .. property_name](self._assistant, self._page, value)
end
function assistant_property_mt:__index(property_name)
	return self._assistant["get_page_" .. property_name](self._assistant, self._page)
end
local assistant_properties_mt = {}
function assistant_properties_mt:__index(page)
	if type(page) == "string" then
		page = self._assistant.child[page]
	end
	return setmetatable({ _assistant = self._assistant, _page = page }, assistant_property_mt)
end
...
Gtk.EntryCompletion._attribute = {
	text_column = { get = Gtk.EntryCompletion.get_text_column, set = Gtk.EntryCompletion.set_text_column },
}
