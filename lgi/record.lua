...
local rawget, assert, select, pairs, type, error, setmetatable = rawget, assert, select, pairs, type, error, setmetatable
...
function record.struct_mt:_element(instance, symbol)
	...
	if symbol == "_native" then
		return symbol, "_internal"
	elseif symbol == "_type" then
		return symbol, "_internal"
	elseif symbol == "_refsink" then
		return symbol, "_internal"
	end
...
function record.struct_mt:_access_field(instance, element, ...)
	...
	if select("#", ...) > 0 then
		if gi.isinfo(element) and element.is_field then
			...
			if type(element) == "table" and (element[2] == 1 or element[2] == 2) then
				subrecord = true
			end
		end
	end
...
function record.load(info)
	local record = component.create(info, info.is_struct and record.struct_mt or record.union_mt)
	...
	if not ctor then
		ctor = info.methods.new
	end

	-- Check, whether ctor is valid.  In order to be valid, it must
	-- return instance of this record.
	if ctor and ctor.type == "function" and ctor.return_type.tag == "interface" and ctor.return_type.interface == info then
		ctor = core.callable.new(ctor)
		record._new = function(typetable, ...)
			return ctor(...)
		end
	end
	return record
