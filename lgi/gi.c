...
gpointer lgi_gi_load_function(lua_State *L, int typetable, const char *name) {
	...
	if (info && GI_IS_FUNCTION_INFO(*info)) {
		g_typelib_symbol(g_base_info_get_typelib(*info), g_function_info_get_symbol(*info), &symbol);
...
static int infos_gc(lua_State *L) {
	Infos *infos = luaL_checkudata(L, 1, LGI_GI_INFOS);
...
static int infos_new(lua_State *L, GIBaseInfo *info, gint count, InfosItemGet item_get) {
...
static const luaL_Reg gi_infos_reg[] = {{"__gc", infos_gc}, {"__len", infos_len}, {"__index", infos_index}, {NULL, NULL}};

static int info_push_transfer(lua_State *L, GITransfer transfer) {
	if (0)
		;
#define H(n1, n2) \
...
static int info_index(lua_State *L) {
	GIBaseInfo **info = luaL_checkudata(L, 1, LGI_GI_INFO);
	const gchar *prop = luaL_checkstring(L, 2);

#define INFOS(n1, n2) else if (strcmp(prop, #n2 "s") == 0) return infos_new(L, *info, g_##n1##_info_get_n_##n2##s(*info), g_##n1##_info_get_##n2);

#define INFOS2(n1, n2, n3) else if (strcmp(prop, #n3) == 0) return infos_new(L, *info, g_##n1##_info_get_n_##n3(*info), g_##n1##_info_get_##n2);
	...
	if (GI_IS_REGISTERED_TYPE_INFO(*info)) {
		if (strcmp(prop, "gtype") == 0) {
			...
			if (strcmp(prop, "type_struct") == 0) {
				return lgi_gi_info_new(L, g_interface_info_get_iface_struct(*info));
			}
...
#undef H
	...
	if (GI_IS_ARG_INFO(*info)) {
		if (strcmp(prop, "direction") == 0) {
			...
			if (dir == GI_DIRECTION_OUT) {
				lua_pushstring(L, g_arg_info_is_caller_allocates(*info) ? "out-caller-alloc" : "out");
			} else {
		...
		if (strcmp(prop, "transfer") == 0) {
			return info_push_transfer(L, g_arg_info_get_ownership_transfer(*info));
		}
		if (strcmp(prop, "optional") == 0) {
			lua_pushboolean(L, g_arg_info_is_optional(*info) || g_arg_info_may_be_null(*info));
			return 1;
		}
	}
	...
	if (GI_IS_PROPERTY_INFO(*info)) {
		if (strcmp(prop, "flags") == 0) {
			...
			return info_push_transfer(L, g_property_info_get_ownership_transfer(*info));
		}
	}
...
#undef H
	...
	if (GI_IS_TYPE_INFO(*info)) {
		...
		if (strcmp(prop, "tag") == 0) {
			...
			if (tag == GI_TYPE_TAG_ARRAY || tag == GI_TYPE_TAG_GLIST || tag == GI_TYPE_TAG_GSLIST || tag == GI_TYPE_TAG_GHASH) {
				lua_newtable(L);
...
#undef H
		...
		} else if (strcmp(prop, "is_zero_terminated") == 0 && tag == GI_TYPE_TAG_ARRAY) {
...
static int info_eq(lua_State *L) {
...
static int info_gc(lua_State *L) {
	GIBaseInfo **info = luaL_checkudata(L, 1, LGI_GI_INFO);
	g_base_info_unref(*info);
	return 0;
}

static const luaL_Reg gi_info_reg[] = {{"__gc", info_gc}, {"__index", info_index}, {"__eq", info_eq}, {NULL, NULL}};

/* Userdata representing symbol resolver of the namespace. */
#define LGI_GI_RESOLVER "lgi.gi.resolver"

static int resolver_index(lua_State *L) {
	gpointer    address;
...
static const luaL_Reg gi_resolver_reg[] = {{"__index", resolver_index}, {NULL, NULL}};

/* Userdata representing namespace in girepository. */
#define LGI_GI_NAMESPACE "lgi.gi.namespace"

static int namespace_len(lua_State *L) {
	const gchar *ns = luaL_checkudata(L, 1, LGI_GI_NAMESPACE);
	lua_pushinteger(L, g_irepository_get_n_infos(NULL, ns));
	return 1;
}

static int namespace_index(lua_State *L) {
	const gchar *ns = luaL_checkudata(L, 1, LGI_GI_NAMESPACE);
	const gchar *prop;
	if (lua_type(L, 2) == LUA_TNUMBER) {
		GIBaseInfo *info = g_irepository_get_info(NULL, ns, lua_tointeger(L, 2) - 1);
		return lgi_gi_info_new(L, info);
	}
...
static int namespace_new(lua_State *L, const gchar *namespace) {
...
static const luaL_Reg gi_namespace_reg[] = {{"__index", namespace_index}, {"__len", namespace_len}, {NULL, NULL}};

/* Lua API: core.gi.require(namespace[, version[, typelib_dir]]) */
static int gi_require(lua_State *L) {
	GError *err              = NULL;
	...
	if (typelib_dir == NULL) {
		...
		typelib = g_irepository_require_private(NULL, typelib_dir, namespace, version, 0, &err);
	}
	if (!typelib) {
		lua_pushboolean(L, 0);
...
static int gi_isinfo(lua_State *L) {
...
static int gi_index(lua_State *L) {
	if (lua_type(L, 2) == LUA_TLIGHTUSERDATA) {
		GType       gtype = (GType)lua_touserdata(L, 2);
		GIBaseInfo *info  = (gtype != G_TYPE_INVALID) ? g_irepository_find_by_gtype(NULL, gtype) : NULL;
		return lgi_gi_info_new(L, info);
...
static const Reg gi_reg[] = {{LGI_GI_INFOS, gi_infos_reg}, {LGI_GI_INFO, gi_info_reg}, {LGI_GI_NAMESPACE, gi_namespace_reg}, {LGI_GI_RESOLVER, gi_resolver_reg}, {NULL, NULL}};

static const luaL_Reg gi_api_reg[] = {{"require", gi_require}, {"isinfo", gi_isinfo}, {NULL, NULL}};
...
gint lgi_field_info_get_offset(GIFieldInfo *info) {
