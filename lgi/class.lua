...
local type, tostring, rawget, rawset, pairs, select, getmetatable, setmetatable, error, assert = type, tostring, rawget, rawset, pairs, select, getmetatable, setmetatable, error, assert
...
local class = {
	class_mt = component.mt:clone("class", { "_virtual", "_property", "_signal", "_method", "_constant", "_field" }),
	interface_mt = component.mt:clone("interface", { "_virtual", "_property", "_signal", "_method", "_constant" }),
}
...
function class.class_mt:is_type_of(instance)
	if type(instance) == "userdata" then
		...
		while instance_type do
			if instance_type == self or instance_type._implements[self._name] == self then
				return true
			end
...
local function load_properties(info)
	return component.get_category(info.properties, nil, function(name)
...
local internals = { _native = true, _type = true, _gtype = true, _class = true, class = true }
function class.class_mt:_element(instance, symbol)
	-- Special handling of internal symbols.
	if instance and internals[symbol] then
...
function class.class_mt:_access_gtype(instance)
	...
	local ti = core.record.new(type_instance, core.object.query(instance, "addr"))
...
function class.load_interface(namespace, info)
	...
	interface._signal = component.get_category(info.signals, nil, load_signal_name, load_signal_name_reverse)
	interface._constant = component.get_category(info.constants, core.constant)
	local type_struct = info.type_struct
	if type_struct then
		interface._virtual = component.get_category(info.vfuncs, nil, load_vfunc_name, load_vfunc_name_reverse)
...
function class.load_class(namespace, info)
	...
	local class = component.create(info, parent and getmetatable(parent) or class.class_mt)
	class._parent = parent
	class._property = load_properties(info)
	class._method = component.get_category(info.methods, load_method)
	class._signal = component.get_category(info.signals, nil, load_signal_name, load_signal_name_reverse)
	class._constant = component.get_category(info.constants, core.constant)
	class._field = component.get_category(info.fields)
	local type_struct = info.type_struct
	if type_struct then
		class._virtual = component.get_category(info.vfuncs, nil, load_vfunc_name, load_vfunc_name_reverse)
		class._class = record.load(type_struct)
		class._class._gtype = class._gtype
		class._class._allow = true
		class._class._parent = parent and parent._class or core.repo.GObject.TypeClass
	end
...
local type_add_interface_static = core.callable.new(GObject.type_add_interface_static)
function class.class_mt:derive(typename, ifaces)
	...
	local class_init_guard, class_init_addr = core.marshal.callback(GObject.ClassInitFunc, class_init)
	...
	local function instance_init(instance)
		...
		if _init then
			...
			_init(core.object.new(core.record.query(instance, "addr"), false, true))
		end
	end
	local instance_init_guard, instance_init_addr = core.marshal.callback(GObject.InstanceInitFunc, instance_init)
	...
	for _, iface in pairs(ifaces or {}) do
		...
		local iface_init_guard, iface_init_addr = core.marshal.callback(GObject.InterfaceInitFunc, iface_init)
...
function class.derived_mt:_element(instance, symbol)
	...
	if instance and symbol == "priv" then
		return symbol, "_priv"
	end
...
function class.derived_mt:__newindex(name, target)
	...
	local _element = (rawget(self, "_element") or rawget(getmetatable(self), "_element"))
	...
	if category == "_virtual" then
		...
		local guard, vfunc = core.marshal.callback(class_struct[name].callable, target)
		override[name] = vfunc
