...
local assert, pcall, setmetatable, getmetatable, pairs, next, rawget, rawset, type, select, error = assert, pcall, setmetatable, getmetatable, pairs, next, rawget, rawset, type, select, error
...
-- symbols.
function component.get_category(children, xform_value, xform_name, xform_name_reverse)
	...
	function mt:__index(requested_name)
		...
		local name = not xform_name and requested_name or xform_name(requested_name)
		if not name then
			return
		end
...
function component.mt:_access(instance, symbol, ...)
	...
	if category then
		...
		if handler then
			return handler(self, instance, element, ...)
		end
	end
...
function component.mt:_access_attribute(instance, element, ...)
	...
	if type(element) == "table" then
		...
		if not element[mode] then
			error(("%s: cannot %s `%s'"):format(self._name, mode == "get" and "read" or "write", element._name or "<unknown>"), 5)
		end
