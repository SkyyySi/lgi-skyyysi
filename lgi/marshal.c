...
static lua_Number check_integer(lua_State *L, int narg, lua_Number val_min, lua_Number val_max) {
...
#else
static lua_Integer check_integer(lua_State *L, int narg, lua_Integer val_min, lua_Integer val_max) {
...
/* Marshals integral types to C.  If requested, makes sure that the
   ...
   input type is. */
static void marshal_2c_int(lua_State *L, GITypeTag tag, GIArgument *val, int narg, gboolean optional, int parent) {
	(void)optional;
	switch (tag) {
#define HANDLE_INT(nameup, namelow, ptrconv, pct, val_min, val_max, ut) \
	case GI_TYPE_TAG_##nameup: \
		...
		if (parent == LGI_PARENT_FORCE_POINTER) \
			val->v_pointer = G##ptrconv##_TO_POINTER((pct)val->v_##namelow); \
		else if (sizeof(g##namelow) <= sizeof(long) && parent == LGI_PARENT_IS_RETVAL) { \
...
#define HANDLE_INT_NOPTR(nameup, namelow, val_min, val_max, ut) \
	case GI_TYPE_TAG_##nameup: \
		...
		if (sizeof(g##namelow) <= sizeof(long) && parent == LGI_PARENT_IS_RETVAL) { \
			ReturnUnion *ru = (ReturnUnion *)val; \
			ru->ut          = ru->arg.v_##namelow; \
		} \
		break
...
#else
		HANDLE_INT_NOPTR(INT64, int64, ((lua_Number)-0x7f00000000000000LL) - 1, 0x7fffffffffffffffLL, s);
...
static void marshal_2lua_int(lua_State *L, GITypeTag tag, GIArgument *val, int parent) {
	switch (tag) {
#define HANDLE_INT(nameupper, namelower, ptrconv, ut) \
	case GI_TYPE_TAG_##nameupper: \
		if (sizeof(g##namelower) <= sizeof(long) && parent == LGI_PARENT_IS_RETVAL) { \
			ReturnUnion *ru       = (ReturnUnion *)val; \
			ru->arg.v_##namelower = (g##namelower)ru->ut; \
		} \
		lua_pushinteger(L, parent == LGI_PARENT_FORCE_POINTER ? GPOINTER_TO_##ptrconv(val->v_pointer) : val->v_##namelower); \
		break;
...
static void array_get_or_set_length(GITypeInfo *ti, gssize *get_length, gssize set_length, GIBaseInfo *ci, void *args) {
...
static gssize array_get_elt_size(GITypeInfo *ti, gboolean force_ptr) {
...
static void array_detach(GArray *array) {
	g_array_free(array, FALSE);
}

static void ptr_array_detach(GPtrArray *array) {
	g_ptr_array_free(array, FALSE);
}

static void byte_array_detach(GByteArray *array) {
	g_byte_array_free(array, FALSE);
}

/* Marshalls array from Lua to C. Returns number of temporary elements
   pushed to the stack. */
static int marshal_2c_array(lua_State *L, GITypeInfo *ti, GIArrayType atype, gpointer *out_array, gssize *out_size, int narg, gboolean optional, GITransfer transfer) {
	GITypeInfo *eti;
	gssize      objlen, esize;
	gint        index, vals = 0, to_pop, eti_guard;
	GITransfer  exfer = (transfer == GI_TRANSFER_EVERYTHING ? GI_TRANSFER_EVERYTHING : GI_TRANSFER_NOTHING);
	...
	if (optional && lua_isnoneornil(L, narg)) {
		...
		*out_array = NULL;
		if (lua_type(L, narg) != LUA_TTABLE && esize == 1 && atype == GI_ARRAY_TYPE_C) {
			size_t size = 0;
			...
			*out_size       = g_type_info_get_array_fixed_size(ti);
			...
			if (*out_size > 0 || zero_terminated) {
				...
				case GI_ARRAY_TYPE_ARRAY:
					array = g_array_sized_new(zero_terminated, TRUE, esize, *out_size);
					g_array_set_size(array, *out_size);
					*lgi_guard_create(L, (GDestroyNotify)(transfer == GI_TRANSFER_EVERYTHING ? array_detach : g_array_unref)) = array;
					break;
				...
				case GI_ARRAY_TYPE_PTR_ARRAY:
					...
					*lgi_guard_create(L, (GDestroyNotify)(transfer == GI_TRANSFER_EVERYTHING ? ptr_array_detach : g_ptr_array_unref)) = array;
					break;

				case GI_ARRAY_TYPE_BYTE_ARRAY:
					array = (GArray *)g_byte_array_sized_new(total_size);
					g_byte_array_set_size((GByteArray *)array, *out_size);
					*lgi_guard_create(L, (GDestroyNotify)(transfer == GI_TRANSFER_EVERYTHING ? byte_array_detach : g_byte_array_unref)) = array;
					break;
				}
				vals = 1;
			}
			...
			for (index = 0; index < objlen; index++) {
			   ...
			   array. */
				to_pop = lgi_marshal_2c(L, eti, NULL, exfer, array->data + index * esize, -1, parent, NULL, NULL);
...
static void marshal_2lua_array(lua_State *L, GITypeInfo *ti, GIDirection dir, GIArrayType atype, GITransfer transfer, gpointer array, gssize size, int parent) {
	GITypeInfo *eti;
	gssize      len = 0, esize;
	...
	if (g_type_info_get_tag(eti) == GI_TYPE_TAG_UINT8) {
		...
		for (index = 0; len < 0 || index < len; index++) {
			...
			lgi_marshal_2lua(L, eti, NULL, dir, (transfer == GI_TRANSFER_EVERYTHING) ? GI_TRANSFER_EVERYTHING : GI_TRANSFER_NOTHING, eval, parent, NULL, NULL);
...
static int marshal_2c_list(lua_State *L, GITypeInfo *ti, GITypeTag list_tag, gpointer *list, int narg, GITransfer transfer) {
	GITypeInfo *eti;
	GITransfer  exfer = (transfer == GI_TRANSFER_EVERYTHING ? GI_TRANSFER_EVERYTHING : GI_TRANSFER_NOTHING);
	gint        index, vals = 0, to_pop, eti_guard;
	GSList    **guard = NULL;
	...
	/* Go from back and prepend to the list, which is cheaper than
	   appending. */
	guard = (GSList **)lgi_guard_create(L, list_tag == GI_TYPE_TAG_GSLIST ? (GDestroyNotify)g_slist_free : (GDestroyNotify)g_list_free);
	while (index > 0) {
	   ...
	   it as pointer to arg. */
		...
		lua_gettable(L, narg);
		to_pop = lgi_marshal_2c(L, eti, NULL, exfer, &eval, -1, LGI_PARENT_FORCE_POINTER, NULL, NULL);
...
static int marshal_2lua_list(lua_State *L, GITypeInfo *ti, GIDirection dir, GITypeTag list_tag, GITransfer xfer, gpointer list) {
	GSList     *i;
	GITypeInfo *eti;
	gint        index, eti_guard;
	...
	for (i = list, index = 0; i != NULL; i = g_slist_next(i)) {
		...
		lgi_marshal_2lua(L, eti, NULL, dir, (xfer == GI_TRANSFER_EVERYTHING) ? GI_TRANSFER_EVERYTHING : GI_TRANSFER_NOTHING, eval, LGI_PARENT_FORCE_POINTER, NULL, NULL);
...
static int marshal_2c_hash(lua_State *L, GITypeInfo *ti, GHashTable **table, int narg, gboolean optional, GITransfer transfer) {
	GITypeInfo  *eti[2];
	GITransfer   exfer = (transfer == GI_TRANSFER_EVERYTHING ? GI_TRANSFER_EVERYTHING : GI_TRANSFER_NOTHING);
	gint         i, vals = 0, guard;
	...
	if (optional && lua_isnoneornil(L, narg)) {
	   ...
	   case something goes wrong during marshalling. */
		guarded_table = (GHashTable **)lgi_guard_create(L, (GDestroyNotify)g_hash_table_destroy);
		vals++;
	   ...
	   according to the type of the key. */
		...
		case GI_TYPE_TAG_DOUBLE:
			return luaL_error(
			    L,
			    "hashtable with float or double is not "
			    "supported");
		default:
		...
		*guarded_table = *table = g_hash_table_new(hash_func, equal_func);
		...
		while (lua_next(L, narg)) {
			...
			for (i = 0; i < 2; i++) {
				vals += lgi_marshal_2c(L, eti[i], NULL, exfer, &eval[i], key_pos + i, LGI_PARENT_FORCE_POINTER, NULL, NULL);
			}
...
static void marshal_2lua_hash(lua_State *L, GITypeInfo *ti, GIDirection dir, GITransfer xfer, GHashTable *hash_table) {
	GHashTableIter iter;
	GITypeInfo    *eti[2];
	gint           i, guard;
	GIArgument     eval[2];
	...
	if (hash_table == NULL) {
	   ...
	   leak it. */
		...
		while (g_hash_table_iter_next(&iter, &eval[0].v_pointer, &eval[1].v_pointer)) {
			/* Marshal key and value to the stack. */
			for (i = 0; i < 2; i++) {
				lgi_marshal_2lua(L, eti[i], NULL, dir, GI_TRANSFER_NOTHING, &eval[i], LGI_PARENT_FORCE_POINTER, NULL, NULL);
			}
...
static void marshal_2lua_error(lua_State *L, GITransfer xfer, GError *err) {
	if (err == NULL) {
		lua_pushnil(L);
	} else {
...
static int marshal_2c_callable(lua_State *L, GICallableInfo *ci, GIArgInfo *ai, gpointer *callback, int narg, gboolean optional, GICallableInfo *argci, void **args) {
	int         nret = 0;
	GIScopeType scope;
	...
	*callback = lgi_closure_create(L, user_data, narg, scope == GI_SCOPE_TYPE_ASYNC);
	return nret;
}

/* Marshalls single value from Lua to GLib/C. */
int lgi_marshal_2c(lua_State *L, GITypeInfo *ti, GIArgInfo *ai, GITransfer transfer, gpointer target, int narg, int parent, GICallableInfo *ci, void **args) {
	int         nret     = 0;
	gboolean    optional = (parent == LGI_PARENT_CALLER_ALLOC) || (ai == NULL || (g_arg_info_is_optional(ai) || g_arg_info_may_be_null(ai)));
	...
	case GI_TYPE_TAG_DOUBLE: {
		...
		lua_Number num = (optional && lua_isnoneornil(L, narg)) ? 0 : luaL_checknumber(L, narg);
	...
	case GI_TYPE_TAG_INTERFACE: {
		...
		case GI_INFO_TYPE_FLAGS:
			...
			marshal_2c_int(L, g_enum_info_get_storage_type(info), arg, narg, optional, parent);
		...
		case GI_INFO_TYPE_UNION: {
		   ...
		   arguments are always passed as pointers. */
			gboolean by_value = parent != LGI_PARENT_FORCE_POINTER && ((!g_type_info_is_pointer(ti) && ai == NULL) || parent == LGI_PARENT_CALLER_ALLOC);

			lgi_type_get_repotype(L, G_TYPE_INVALID, info);
			lgi_record_2c(L, narg, target, by_value, transfer != GI_TRANSFER_NOTHING, optional, FALSE);
			break;
		}

		case GI_INFO_TYPE_OBJECT:
		case GI_INFO_TYPE_INTERFACE: {
			arg->v_pointer = lgi_object_2c(L, narg, g_registered_type_info_get_g_type(info), optional, FALSE, transfer != GI_TRANSFER_NOTHING);
			break;
		}

		case GI_INFO_TYPE_CALLBACK:
			nret = marshal_2c_callable(L, info, ai, &arg->v_pointer, narg, optional, ci, args);
			break;
	...
	case GI_TYPE_TAG_ARRAY: {
		...
		nret              = marshal_2c_array(L, ti, atype, &arg->v_pointer, &size, narg, optional, transfer);
	...
	case GI_TYPE_TAG_GHASH:
		nret = marshal_2c_hash(L, ti, (GHashTable **)&arg->v_pointer, narg, optional, transfer);
		break;
		...
		if (g_type_info_is_pointer(ti)) {
			...
			if (lua_type(L, narg) == LUA_TSTRING) {
				...
				if (type == LUA_TLIGHTUSERDATA) {
					...
					if (!arg->v_pointer) {
						/* Check object. */
						arg->v_pointer = lgi_object_2c(L, narg, G_TYPE_INVALID, FALSE, TRUE, FALSE);
						if (!arg->v_pointer) {
							/* Check any kind of record. */
							lua_pushnil(L);
							lgi_record_2c(L, narg, &arg->v_pointer, FALSE, FALSE, FALSE, TRUE);
						}
					}
				}
			}
...
gboolean lgi_marshal_2c_caller_alloc(lua_State *L, GITypeInfo *ti, GIArgument *val, int pos) {
	...
	case GI_TYPE_TAG_ARRAY: {
		if (g_type_info_get_array_type(ti) == GI_ARRAY_TYPE_C) {
			...
			if (pos == 0) {
				...
				elt_size = array_get_elt_size(g_type_info_get_param_type(ti, 0), FALSE);
				...
				array_guard  = lgi_guard_create(L, (GDestroyNotify)g_array_unref);
				*array_guard = g_array_sized_new(FALSE, FALSE, elt_size, size);
				...
				marshal_2lua_array(L, ti, GI_DIRECTION_OUT, GI_ARRAY_TYPE_ARRAY, GI_TRANSFER_EVERYTHING, *array_guard, -1, pos);
...
void lgi_marshal_2lua(lua_State *L, GITypeInfo *ti, GIArgInfo *ai, GIDirection dir, GITransfer transfer, gpointer source, int parent, GICallableInfo *ci, void *args) {
	...
	case GI_TYPE_TAG_DOUBLE:
		...
		lua_pushnumber(L, (tag == GI_TYPE_TAG_FLOAT) ? arg->v_float : arg->v_double);
		break;

	case GI_TYPE_TAG_UTF8:
	case GI_TYPE_TAG_FILENAME: {
		gchar *str = (parent == LGI_PARENT_FORCE_POINTER) ? arg->v_pointer : arg->v_string;
	...
	case GI_TYPE_TAG_INTERFACE: {
		...
		case GI_INFO_TYPE_FLAGS:
			...
			marshal_2lua_int(L, g_enum_info_get_storage_type(info), arg, parent);
		...
		case GI_INFO_TYPE_STRUCT:
		case GI_INFO_TYPE_UNION: {
			gboolean by_ref = parent == LGI_PARENT_FORCE_POINTER || g_type_info_is_pointer(ti);
			if (parent < LGI_PARENT_CALLER_ALLOC && by_ref) {
				parent = 0;
			}
			lgi_type_get_repotype(L, G_TYPE_INVALID, info);
			lgi_record_2lua(L, by_ref ? arg->v_pointer : source, own, parent);
			break;
		}
...
int lgi_marshal_field(lua_State *L, gpointer object, gboolean getmode, int parent_arg, int field_arg, int val_arg) {
	GITypeInfo *ti;
	int         to_remove, nret;
	GIBaseInfo *pi = NULL;
	gpointer    field_addr;
	...
	if (lgi_udata_test(L, field_arg, LGI_GI_INFO)) {
		...
		if ((flags & (getmode ? GI_FIELD_IS_READABLE : GI_FIELD_IS_WRITABLE)) == 0) {
			...
			if (!lua_toboolean(L, -1)) {
				...
				return luaL_error(L, "%s: field `%s' is not %s", lua_tostring(L, -1), g_base_info_get_name(*fi), getmode ? "readable" : "writable");
			}
			lua_pop(L, 1);
		}
	   ...
	   it according to 'kind' */
		...
		case 2: {
			...
			if (getmode) {
				...
				g_assert(kind == 1);
				lgi_record_2c(L, val_arg, arg->v_pointer, FALSE, TRUE, FALSE, FALSE);
				return 0;
			}
			break;
		}
		...
		case 3: {
			...
			if (getmode) {
				...
				lgi_marshal_2lua(L, ti, NULL, GI_DIRECTION_OUT, GI_TRANSFER_NOTHING, field_addr, 0, NULL, NULL);
				...
				lgi_marshal_2c(L, ti, NULL, GI_TRANSFER_NOTHING, field_addr, val_arg, 0, NULL, NULL);
				lua_pop(L, 2);
				return 0;
			}
		}
	...
	if (getmode) {
		lgi_marshal_2lua(L, ti, NULL, GI_DIRECTION_OUT, GI_TRANSFER_NOTHING, field_addr, parent_arg, pi, object);
		nret = 1;
	} else {
		lgi_marshal_2c(L, ti, NULL, GI_TRANSFER_EVERYTHING, field_addr, val_arg, 0, NULL, NULL);
		nret = 0;
	}
...
int lgi_marshal_access(lua_State *L, gboolean getmode, int compound_arg, int element_arg, int val_arg) {
...
static int marshal_container_marshaller(lua_State *L) {
	GValue      *value;
	GITypeInfo **ti;
	GITypeTag    tag;
	GITransfer   transfer;
	...
	case GI_TYPE_TAG_ARRAY: {
		...
		if (get_mode) {
			...
			marshal_2lua_array(L, *ti, GI_DIRECTION_OUT, atype, transfer, data, size, 0);
		} else {
			nret = marshal_2c_array(L, *ti, atype, &data, &size, 3, FALSE, transfer);
			if (lua_type(L, 2) == LUA_TTABLE) {
	...
	case GI_TYPE_TAG_GHASH:
		if (get_mode) {
			...
			nret = marshal_2c_hash(L, *ti, (GHashTable **)&data, 3, FALSE, transfer);
		}
		break;
...
static int marshal_container(lua_State *L) {
	GIBaseInfo **info     = luaL_checkudata(L, 1, LGI_GI_INFO);
	GITypeTag    tag      = g_type_info_get_tag(*info);
	GITransfer   transfer = luaL_checkoption(L, 2, transfers[0], transfers);
	if (tag == GI_TYPE_TAG_ARRAY || tag == GI_TYPE_TAG_GHASH || tag == GI_TYPE_TAG_GSLIST || tag == GI_TYPE_TAG_GLIST) {
		lua_pushvalue(L, 1);
...
static int marshal_fundamental_marshaller(lua_State *L) {
	gpointer obj;
	gboolean get_mode = lua_isnone(L, 3);
	GValue  *value;
	lgi_type_get_repotype(L, G_TYPE_VALUE, NULL);
	...
	if (get_mode) {
		...
		GIObjectInfoGetValueFunction get_value = lua_touserdata(L, lua_upvalueindex(1));
		obj                                    = get_value(value);
		lgi_object_2lua(L, obj, FALSE, FALSE);
		return 1;
	} else {
		/* Set fundamental to value. */
		GIObjectInfoSetValueFunction set_value = lua_touserdata(L, lua_upvalueindex(2));
...
static int marshal_fundamental(lua_State *L) {
	/* Find associated baseinfo. */
	GIBaseInfo *info = g_irepository_find_by_gtype(NULL, lgi_type_get_gtype(L, 1));
	if (info) {
		lgi_gi_info_new(L, info);
		if (GI_IS_OBJECT_INFO(info) && g_object_info_get_fundamental(info)) {
			GIObjectInfoGetValueFunction get_value = lgi_object_get_function_ptr(info, g_object_info_get_get_value_function);
			GIObjectInfoSetValueFunction set_value = lgi_object_get_function_ptr(info, g_object_info_get_set_value_function);
...
static int marshal_argument(lua_State *L) {
	GITypeInfo **info;
	GITransfer   transfer;
	GIArgument  *arg;
	...
	if (lua_isnone(L, 4)) {
		lgi_marshal_2lua(L, *info, NULL, GI_DIRECTION_IN, transfer, arg, 0, NULL, NULL);
		return 1;
	} else {
		lua_pop(L, lgi_marshal_2c(L, *info, NULL, transfer, arg, 4, 0, NULL, NULL));
		return 0;
	}
}

static int marshal_callback(lua_State *L) {
...
static void gclosure_destroy(gpointer user_data, GClosure *closure) {
	(void)closure;
...
static int marshal_closure_invoke(lua_State *L) {
	GClosure *closure;
...
*/
static int marshal_closure_set_marshal(lua_State *L) {
	GClosure       *closure;
	gpointer        user_data;
	GClosureMarshal marshal;
	GIBaseInfo     *ci;
...
static int marshal_typeinfo(lua_State *L) {
	GIBaseInfo **info = luaL_checkudata(L, 1, LGI_GI_INFO);
...
static const struct luaL_Reg marshal_api_reg[] = {{"container", marshal_container}, {"fundamental", marshal_fundamental}, {"argument", marshal_argument}, {"callback", marshal_callback}, {"closure_set_marshal", marshal_closure_set_marshal}, {"closure_invoke", marshal_closure_invoke}, {"typeinfo", marshal_typeinfo}, {NULL, NULL}};
