...
local type, rawget, next, pairs, require, pcall, setmetatable, assert = type, rawget, next, pairs, require, pcall, setmetatable, assert
...
typeloader["function"] = function(namespace, info)
...
local namespace = {
	mt = {
		_categories = { "_class", "_interface", "_struct", "_union", "_enum", "_function", "_constant" },
		_category_mt = {},
	},
}
...
function namespace.mt:__index(symbol)
	...
	if precondition then
		...
		if not next(preconditions) then
	...
	if loader then
		...
		if val then
			...
			-- other symbol.
			if not cat[symbol] then
				cat[symbol] = val
			end
		elseif info.is_gtype_struct then
			-- If we have XxxClass name, try to lookup class structure of
			-- the Xxx object.
			local class = (symbol:match("^(%w+)Class$") or symbol:match("^(%w+)Iface$") or symbol:match("^(%w+)Interface$"))
			if class then
				class = self[class]
				if class then
					val = class._class
...
function namespace.require(name, version)
	...
	if not ns then
		ns = setmetatable({ _name = name, _version = ns_info.version, _dependencies = ns_info.dependencies }, namespace.mt)
		core.repo[name] = ns
