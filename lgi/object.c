...
static gpointer object_check(lua_State *L, int narg) {
...
/* Walks given type and tries to find the closest known match of the
   ...
   G_TYPE_INVALID. */
static GType object_type(lua_State *L, GType gtype) {
...
static int object_type_error(lua_State *L, int narg, GType gtype) {
	GType found_gtype;
	...
	if (found_gtype != G_TYPE_INVALID) {
		...
		lua_pushfstring(L, gtype == found_gtype ? "%s" : "%s(%s)", lua_tostring(L, -1), g_type_name(gtype));
	} else {
	...
	lua_pushfstring(L, "%s expected, got %s", lua_tostring(L, -2), lua_tostring(L, -1));
	return luaL_argerror(L, narg, lua_tostring(L, -1));
}

static gpointer object_get(lua_State *L, int narg) {
...
gpointer lgi_object_get_function_ptr(GIObjectInfo *info, const gchar *(*getter)(GIObjectInfo *)) {
	gpointer func = NULL;
	...
	while (info != NULL) {
		...
		if (func_name && g_typelib_symbol(g_base_info_get_typelib(info), func_name, &func)) {
...
static gpointer object_load_function(lua_State *L, GType gtype, const gchar *name) {
	gpointer func = NULL;
...
static gboolean object_refsink(lua_State *L, gpointer obj, gboolean no_sink) {
	...
	if (info != NULL && g_object_info_get_fundamental(info)) {
		GIObjectInfoRefFunction ref = lgi_object_get_function_ptr(info, g_object_info_get_ref_function);
		g_base_info_unref(info);
		if (ref != NULL) {
			ref(obj);
			return TRUE;
	...
	gpointer (*refsink_func)(gpointer) = object_load_function(L, gtype, "_refsink");
	if (refsink_func) {
		refsink_func(obj);
		return TRUE;
	}
...
static void object_unref(lua_State *L, gpointer obj) {
	GType gtype = G_TYPE_FROM_INSTANCE(obj);
	if (G_TYPE_IS_OBJECT(gtype)) {
		g_object_unref(obj);
		return;
	...
	if (info != NULL && g_object_info_get_fundamental(info)) {
		GIObjectInfoUnrefFunction unref = lgi_object_get_function_ptr(info, g_object_info_get_unref_function);
		g_base_info_unref(info);
		if (unref != NULL) {
			unref(obj);
			return;
...
static int object_tostring(lua_State *L) {
	...
	lua_pushfstring(L, "lgi.obj %p:%s(%s)", obj, lua_tostring(L, -1), g_type_name(gtype));
	return 1;
}

gpointer lgi_object_2c(lua_State *L, int narg, GType gtype, gboolean optional, gboolean nothrow, gboolean transfer) {
	gpointer obj;
	...
	if (!nothrow && (!obj || (gtype != G_TYPE_INVALID && !g_type_is_a(G_TYPE_FROM_INSTANCE(obj), gtype)))) {
...
int lgi_object_2lua(lua_State *L, gpointer obj, gboolean own, gboolean no_sink) {
...
static int object_access(lua_State *L) {
...
/* Registration table. */
static const luaL_Reg object_mt_reg[] = {{"__gc", object_gc}, {"__tostring", object_tostring}, {"__index", object_access}, {"__newindex", object_access}, {NULL, NULL}};
...
static int object_query(lua_State *L) {
...
static int object_field(lua_State *L) {
...
static void object_data_destroy(gpointer user_data) {
...
static int object_env_guard_gc(lua_State *L) {
...
static int object_env(lua_State *L) {
	ObjectData *data;
	gpointer    obj = object_get(L, 1);
	if (!G_IS_OBJECT(obj)) {
	...
	if (!lua_isnil(L, -1)) {
	   ...
	   object's qdata. */
		...
		g_object_set_qdata_full(G_OBJECT(obj), guard->id, data, object_data_destroy);
		lua_pop(L, 2);
	}
...
static int object_new(lua_State *L) {
	if (lua_islightuserdata(L, 1)) {
		...
		return lgi_object_2lua(L, lua_touserdata(L, 1), lua_toboolean(L, 2), lua_toboolean(L, 3));
	} else {
		...
		*lgi_guard_create(L, (GDestroyNotify)g_base_info_unref) = gparam_info;
		...
		return lgi_object_2lua(L, g_object_newv(gtype, size, params), TRUE, FALSE);
	}
}

/* Object API table. */
static const luaL_Reg object_api_reg[] = {{"query", object_query}, {"field", object_field}, {"new", object_new}, {"env", object_env}, {NULL, NULL}};
