...
gpointer lgi_record_new(lua_State *L, int count, gboolean alloc) {
	...
	/* Allocate new userdata for record object, attach proper
	   metatable. */
	record = lua_newuserdata(L, G_STRUCT_OFFSET(Record, data) + (alloc ? 0 : size));
...
static void record_free(lua_State *L, Record *record, int narg) {
	GType gtype;
	...
	for (;;) {
		...
		if (G_TYPE_IS_BOXED(gtype)) {
			...
			void (*free_func)(gpointer) = lgi_gi_load_function(L, -1, "_free");
			if (free_func) {
		...
		if (lua_isnil(L, -1)) {
			...
			g_warning("unable to free record %s, leaking it", lua_tostring(L, -1));
			lua_pop(L, 2);
			break;
		}
	}
	   ...
	   cache. */
		...
		if (!own) {
			/* Check, whether refrepo table specifies custom _refsink
			   function. */
			void (*refsink_func)(gpointer) = lgi_gi_load_function(L, -4, "_refsink");
			if (refsink_func) {
				refsink_func(addr);
				own = TRUE;
			}
...
static Record *record_check(lua_State *L, int narg) {
...
static int record_error(lua_State *L, int narg, const gchar *expected_name) {
	luaL_checkstack(L, 2, "");
	lua_pushstring(L, lua_typename(L, lua_type(L, narg)));
	lua_pushfstring(L, "%s expected, got %s", expected_name ? expected_name : "lgi.record", lua_tostring(L, -1));
	return luaL_argerror(L, narg, lua_tostring(L, -1));
}

/* Similar to record_check, but throws in case of failure. */
static Record *record_get(lua_State *L, int narg) {
...
void lgi_record_2c(lua_State *L, int narg, gpointer target, gboolean by_value, gboolean own, gboolean optional, gboolean nothrow) {
	Record *record = NULL;
		...
		*(gpointer *)target = record ? record->addr : NULL;
		if (record && own) {
			...
			if (G_LIKELY(record->store == RECORD_STORE_ALLOCATED)) {
				/* Check, whether refrepo table specifies custom _refsink
			   function. */
				void (*refsink_func)(gpointer) = lgi_gi_load_function(L, narg, "_refsink");
				if (refsink_func) {
		...
		if (record) {
			...
			void (*copy_func)(gpointer, gpointer) = lgi_gi_load_function(L, -1, "_copy");
			if (copy_func) {
...
static int record_gc(lua_State *L) {
	Record *record = record_get(L, 1);

	if (record->store == RECORD_STORE_EMBEDDED || record->store == RECORD_STORE_NESTED) {
...
static int record_access(lua_State *L) {
...
static int record_len(lua_State *L) {
	...
	if (lua_isnil(L, -1)) {
		lua_getfield(L, -2, "_name");
		return luaL_error(L, "`%s': attempt to get length", lua_tostring(L, -1));
	}
	lua_pushvalue(L, 1);
	lua_call(L, 1, 1);
	return 1;
...
static const struct luaL_Reg record_meta_reg[] = {{"__gc", record_gc}, {"__tostring", record_tostring}, {"__index", record_access}, {"__newindex", record_access}, {"__len", record_len}, {NULL, NULL}};
...
/* Implements generic record creation. Creates new record instance,
   ...
   dies). */
static int record_new(lua_State *L) {
	if (lua_isnoneornil(L, 2)) {
		...
		gpointer addr = (lua_type(L, 2) == LUA_TLIGHTUSERDATA) ? addr = lua_touserdata(L, 2) : (gpointer)luaL_checkinteger(L, 2);
...
static int record_query(lua_State *L) {
	Record *record;
...
static int record_field(lua_State *L) {
	gboolean getmode;
	Record  *record;
...
static int record_cast(lua_State *L) {
	Record *record = record_get(L, 1);
...
static int record_fromarray(lua_State *L) {
	Record  *record = record_get(L, 1);
...
/* Changes ownership mode or repotable of the record.
   ...
   - 'repotable' record repotable to which this instance should be
     reassigned. */
static int record_set(lua_State *L) {
	Record *record = record_get(L, 1);
...
static const struct luaL_Reg record_api_reg[] = {{"new", record_new}, {"query", record_query}, {"field", record_field}, {"cast", record_cast}, {"fromarray", record_fromarray}, {"set", record_set}, {NULL, NULL}};

/* Workaround for g_value_unset(), which complains when invoked on
   uninitialized GValue instance. */
static void record_value_unset(GValue *value) {
	if (G_IS_VALUE(value)) {
		g_value_unset(value);
	}
}
...
static void record_value_copy(const GValue *src, GValue *dest) {
