...
static ffi_type *get_simple_ffi_type(GITypeTag tag) {
	ffi_type *ffi;
	switch (tag) {
...
static ffi_type *get_ffi_type(Param *param) {
	switch (param->kind) {
	case PARAM_KIND_RECORD:
		return &ffi_type_pointer;

	case PARAM_KIND_ENUM:
		return param->ti ? get_simple_ffi_type(g_type_info_get_tag(param->ti)) : &ffi_type_sint;
	...
	ffi_type *ffi = g_type_info_is_pointer(param->ti) ? &ffi_type_pointer : get_simple_ffi_type(tag);
	if (ffi == NULL) {
...
static void callable_mark_array_length(Callable *callable, GITypeInfo *ti) {
	gint arg;
	if (g_type_info_get_tag(ti) == GI_TYPE_TAG_ARRAY && g_type_info_get_array_type(ti) == GI_ARRAY_TYPE_C) {
...
static void callable_param_init(Param *param) {
	param->ti                    = NULL;
...
static Callable *callable_allocate(lua_State *L, int nargs, ffi_type ***ffi_args) {
	int argi;

	/* Create userdata structure. */
	luaL_checkstack(L, 2, NULL);
	Callable *callable = lua_newuserdata(L, sizeof(Callable) + sizeof(ffi_type) * (nargs + 2) + sizeof(Param) * nargs);
...
static Param *callable_get_param(Callable *callable, gint n) {
	Param *param;
...
int lgi_callable_create(lua_State *L, GICallableInfo *info, gpointer addr) {
	Callable  *callable;
	...
	if (GI_IS_FUNCTION_INFO(info)) {
		...
		if ((flags & GI_FUNCTION_IS_METHOD) != 0 && (flags & GI_FUNCTION_IS_CONSTRUCTOR) == 0) {
			callable->has_self = 1;
		}
		...
		if (!g_typelib_symbol(g_base_info_get_typelib(info), symbol, &callable->address)) {
			/* Fail with the error message. */
			return luaL_error(L, "could not locate %s(%s): %s", lua_tostring(L, -3), symbol, g_module_error());
		}
	...
	for (argi = 0; argi < nargs; argi++, ffi_arg++) {
		...
		*ffi_arg = (param->dir == GI_DIRECTION_IN) ? get_ffi_type(param) : &ffi_type_pointer;
	   ...
	   value). */
		if (param->dir != GI_DIRECTION_IN && g_type_info_get_tag(callable->retval.ti) == GI_TYPE_TAG_BOOLEAN) {
	...
	if (!GLIB_CHECK_VERSION(2, 30, 0) && !strcmp(g_base_info_get_namespace(info), "GObject") && !strcmp(g_base_info_get_name(info), "ClosureMarshal")) {
	...
	/* Create ffi_cif. */
	if (ffi_prep_cif(&callable->cif, FFI_DEFAULT_ABI, callable->has_self + nargs + callable->throws, ffi_retval, ffi_args) != FFI_OK) {
		lua_concat(L, lgi_type_get_name(L, callable->info));
		return luaL_error(L, "ffi_prep_cif for `%s' failed", lua_tostring(L, -1));
	}

	return 1;
}

static int callable_param_get_kind(lua_State *L) {
	...
	if (lgi_udata_test(L, -1, LGI_GI_INFO)) {
		...
		if (!lua_isnil(L, -1)) {
			...
			if (!lua_isnil(L, -1)) {
				...
				if (g_strcmp0(type, "struct") == 0 || g_strcmp0(type, "union") == 0) {
					kind = PARAM_KIND_RECORD;
				} else if (g_strcmp0(type, "enum") == 0 || g_strcmp0(type, "flags") == 0) {
...
static void callable_param_parse(lua_State *L, Param *param) {
	...
	if (kind == -1) {
		...
		param->transfer = lua_toboolean(L, -1) ? GI_TRANSFER_EVERYTHING : GI_TRANSFER_NOTHING;
		lua_pop(L, 1);
...
int lgi_callable_parse(lua_State *L, int info, gpointer addr) {
	...
	for (i = 0; i < nargs; i++) {
		...
		ffi_args[i] = (callable->params[i].dir == GI_DIRECTION_IN) ? get_ffi_type(&callable->params[i]) : &ffi_type_pointer;
	}
	...
	/* Create ffi_cif. */
	if (ffi_prep_cif(&callable->cif, FFI_DEFAULT_ABI, nargs + callable->throws, ffi_retval, ffi_args) != FFI_OK) {
...
static Callable *callable_get(lua_State *L, int narg) {
	luaL_checkstack(L, 3, "");
	...
	lua_pushfstring(L, "expected lgi.callable, got %s", lua_typename(L, lua_type(L, narg)));
	luaL_argerror(L, narg, lua_tostring(L, -1));
	return NULL;
}

static void callable_param_destroy(Param *param) {
	if (param->ti) {
...
static void callable_describe(lua_State *L, Callable *callable, FfiClosure *closure) {
	luaL_checkstack(L, 2, "");
	...
	if (callable->info) {
		lua_pushfstring(L, "lgi.%s (%s): ", (GI_IS_FUNCTION_INFO(callable->info) ? "fun" : (GI_IS_SIGNAL_INFO(callable->info) ? "sig" : (GI_IS_VFUNC_INFO(callable->info) ? "vfn" : "cbk"))), lua_tostring(L, -1));
		lua_concat(L, lgi_type_get_name(L, callable->info) + 1);
	} else {
		lua_getfenv(L, 1);
		lua_rawgeti(L, -1, 0);
		lua_replace(L, -2);
		lua_pushfstring(L, "lgi.efn (%s): %s", lua_tostring(L, -2), lua_tostring(L, -1));
		lua_replace(L, -2);
	}
...
static int callable_param_2c(lua_State *L, Param *param, int narg, int parent, GIArgument *arg, int callable_index, Callable *callable, void **args) {
	int nret = 0;
	...
	if (param->kind != PARAM_KIND_RECORD) {
		if (param->ti) {
			nret = lgi_marshal_2c(L, param->ti, param->has_arg_info ? &param->ai : NULL, param->transfer, arg, narg, parent, callable->info, args + callable->has_self);
		} else {
			union {
				GIArgument arg;
				int        i;
		...
		lua_getfenv(L, callable_index);
		lua_rawgeti(L, -1, param->repotype_index);
		lgi_record_2c(L, narg, &arg->v_pointer, FALSE, param->transfer != GI_TRANSFER_NOTHING, TRUE, FALSE);
		lua_pop(L, 1);
	}
...
static void callable_param_2lua(lua_State *L, Param *param, GIArgument *arg, int parent, int callable_index, Callable *callable, void **args) {
	if (param->kind != PARAM_KIND_RECORD) {
		if (param->ti) {
			lgi_marshal_2lua(L, param->ti, callable->info ? &param->ai : NULL, param->dir, param->transfer, arg, parent, callable->info, args + callable->has_self);
		} else {
			union {
				GIArgument arg;
				ffi_sarg   i;
	...
	if (param->kind == PARAM_KIND_RECORD) {
		/* Marshal record according to custom information. */
		lgi_record_2lua(L, arg->v_pointer, param->transfer != GI_TRANSFER_NOTHING, parent);
		lua_remove(L, -2);
	} else {
...
static int callable_call(lua_State *L) {
	...
	if (callable->has_self) {
		...
		if (type == GI_INFO_TYPE_OBJECT || type == GI_INFO_TYPE_INTERFACE) {
			args[0].v_pointer = lgi_object_2c(L, 2, g_registered_type_info_get_g_type(parent), FALSE, FALSE, FALSE);
			nret++;
		} else {
	...
	for (i = 0; i < callable->nargs; i++, param++) {
		if (!param->internal) {
			...
			if (param->dir != GI_DIRECTION_OUT) {
				nret += callable_param_2c(L, param, lua_argi++, 0, &args[argi], 1, callable, ffi_args);
			}
			/* Special handling for out/caller-alloc structures; we have to
			   manually pre-create them and store them on the stack. */
			else if (callable->info && g_arg_info_is_caller_allocates(&param->ai) && lgi_marshal_2c_caller_alloc(L, param->ti, &args[argi], 0)) {
	...
	/* Handle return value. */
	nret = 0;
	if (!callable->ignore_retval && (callable->retval.ti == NULL || (g_type_info_get_tag(callable->retval.ti) != GI_TYPE_TAG_VOID || g_type_info_is_pointer(callable->retval.ti)))) {
		callable_param_2lua(L, &callable->retval, &retval, LGI_PARENT_IS_RETVAL, 1, callable, ffi_args);
		nret++;
	...
	for (i = 0; i < callable->nargs; i++, param++) {
		if (!param->internal && param->dir != GI_DIRECTION_IN) {
			if (callable->info && g_arg_info_is_caller_allocates(&param->ai) && lgi_marshal_2c_caller_alloc(L, param->ti, NULL, -caller_allocated - nret)) {
				/* Caller allocated parameter is already marshalled and
				   lying on the stack. */
				caller_allocated--;
			} else {
				/* Marshal output parameter. */
				callable_param_2lua(L, param, &args[i + callable->has_self], 0, 1, callable, ffi_args);
				lua_insert(L, -caller_allocated - 1);
			}
...
static int callable_index(lua_State *L) {
	Callable    *callable = callable_get(L, 1);
	...
	if (g_strcmp0(verb, "info") == 0) {
		...
		for (i = 0, param = callable->params; i < callable->nargs; i++, param++) {
			if (!param->internal) {
				...
				/* Add in.out info. */
				if (param->dir == GI_DIRECTION_IN || param->dir == GI_DIRECTION_INOUT) {
					lua_pushboolean(L, 1);
					lua_setfield(L, -2, "in");
				}
				if (param->dir == GI_DIRECTION_OUT || param->dir == GI_DIRECTION_INOUT) {
					lua_pushboolean(L, 1);
...
static int callable_newindex(lua_State *L) {
	Callable *callable = callable_get(L, 1);
...
static const struct luaL_Reg callable_reg[] = {{"__gc", callable_gc}, {"__tostring", callable_tostring}, {"__call", callable_call}, {"__index", callable_index}, {"__newindex", callable_newindex}, {NULL, NULL}};

static int marshal_arguments(lua_State *L, void **args, int callable_index, Callable *callable) {
	Param *param;
	int    npos = 0, i;
	...
	for (i = 0; i < callable->nargs; ++i, ++param) {
		if (!param->internal && param->dir != GI_DIRECTION_OUT) {
			if G_LIKELY (i != 3 || !callable->is_closure_marshal) {
				...
				callable_param_2lua(L, param, real_arg, 0, callable_index, callable, args + callable->has_self);
			} else {
...
static void marshal_return_values(lua_State *L, void *ret, void **args, int callable_index, Callable *callable, int npos) {
	int       to_pop, i;
	GITypeTag tag;
	Param    *param;
	...
	if (tag != GI_TYPE_TAG_VOID || g_type_info_is_pointer(callable->retval.ti)) {
		if (callable->ignore_retval) {
			...
			*(ffi_sarg *)ret = lua_isnoneornil(L, npos) ? FALSE : TRUE;
			...
			to_pop = callable_param_2c(L, &callable->retval, npos, LGI_PARENT_IS_RETVAL, ret, callable_index, callable, args + callable->has_self);
			if (to_pop != 0) {
				g_warning("cbk `%s.%s': return (transfer none) %d, unsafe!", g_base_info_get_namespace(callable->info), g_base_info_get_name(callable->info), to_pop);
				lua_pop(L, to_pop);
			}
	...
	for (i = 0; i < callable->nargs; ++i, ++param) {
		if (!param->internal && param->dir != GI_DIRECTION_IN) {
			...
			gboolean  caller_alloc = callable->info && g_arg_info_is_caller_allocates(&param->ai) && g_type_info_get_tag(param->ti) == GI_TYPE_TAG_INTERFACE;
			to_pop                 = callable_param_2c(L, param, npos, caller_alloc ? LGI_PARENT_CALLER_ALLOC : 0, *arg, callable_index, callable, args + callable->has_self);
			if (to_pop != 0) {
				g_warning("cbk %s.%s: arg `%s' (transfer none) %d, unsafe!", g_base_info_get_namespace(callable->info), g_base_info_get_name(callable->info), g_base_info_get_name(&param->ai), to_pop);
				lua_pop(L, to_pop);
			}
...
static void marshal_return_error(lua_State *L, void *ret, void **args, Callable *callable) {
	/* If the function is expected to return errors, create proper
	   error. */
	GError **err = ((GIArgument *)args[callable->has_self + callable->nargs])->v_pointer;
...
/* Closure callback, called by libffi when C code wants to invoke Lua
   callback. */
static void closure_callback(ffi_cif *cif, void *ret, void **args, void *closure_arg) {
	Callable        *callable;
	int              callable_index;
	...
	if (call) {
		if (callable->throws) {
			...
			g_warning("Error raised while calling '%s': %s", lua_tostring(L, -1), lua_tostring(L, -2));
			lua_pop(L, 2);
		}
	} else {
...
gpointer lgi_closure_allocate(lua_State *L, int count) {
	...
	FfiClosureBlock *block       = ffi_closure_alloc(offsetof(FfiClosureBlock, ffi_closures) + (--count * sizeof(FfiClosure *)), &call_addr);
	...
	for (i = 0; i < count; ++i) {
		block->ffi_closures[i]            = ffi_closure_alloc(sizeof(FfiClosure), &call_addr);
...
gpointer lgi_closure_create(lua_State *L, gpointer user_data, int target, gboolean autodestroy) {
	FfiClosureBlock *block = user_data;
	FfiClosure      *closure;
	Callable        *callable;
	gpointer         call_addr;
	...
	/* Create closure. */
	if (ffi_prep_closure_loc(&closure->ffi_closure, &callable->cif, closure_callback, closure, call_addr) != FFI_OK) {
		lua_concat(L, lgi_type_get_name(L, callable->info));
...
static int callable_new(lua_State *L) {
	gpointer addr = lua_touserdata(L, 2);
	if (lua_istable(L, 1)) {
		return lgi_callable_parse(L, 1, addr);
	} else {
		return lgi_callable_create(L, *(GICallableInfo **)luaL_checkudata(L, 1, LGI_GI_INFO), addr);
	}
}

/* Callable module public API table. */
static const luaL_Reg callable_api_reg[] = {{"new", callable_new}, {NULL, NULL}};
