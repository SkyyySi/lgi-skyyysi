...
local setmetatable, pairs, type, select = setmetatable, pairs, type, select
...
function enum.load(info, meta)
	...
	if info.methods then
		enum_type._method = component.get_category(info.methods, core.callable.new)
	else
...
function enum.bitflags_mt:_element(instance, value)
	if type(value) == "number" then
		...
		for name, flag in pairs(self) do
			if type(flag) == "number" and name:sub(1, 1) ~= "_" and band(value, flag) == flag then
				result[name] = true
