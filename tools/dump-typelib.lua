...
local infos = {}
infos.base = { attrs = { "name", "namespace", "type", "deprecated" }, cats = {} }
...
function infos.base:derive(attrs, cats)
	local new_attrs = {}
	for _, val in ipairs(self.attrs) do
		new_attrs[#new_attrs + 1] = val
	end
	for _, val in ipairs(attrs or {}) do
		new_attrs[#new_attrs + 1] = val
	end
	local new_cats = {}
	for _, val in ipairs(self.cats) do
		new_cats[#new_cats + 1] = val
	end
	for _, val in ipairs(cats or {}) do
		new_cats[#new_cats + 1] = val
	end
...
function infos.base:get(name, depth)
	...
	if gi.isinfo(item) then
		item = infos.new(item)
		if depth then
			item = item:dump(depth)
		end
	else
		for _, cat in pairs(self.cats) do
			if cat == name then
				item = infos.category.new(item)
			end
		end
	end
	return item
...
infos.type = infos.base:derive({ "tag", "is_basic", "interface", "array_type", "is_zero_terminated", "array_length", "fixed_size", "is_pointer" }, { "params" })
...
infos.object = infos.registered:derive({ "parent", "type_struct" }, { "interfaces", "fields", "vfuncs", "methods", "constants", "properties", "signals" })
infos.interface = infos.registered:derive({ "type_struct" }, { "prerequisites", "vfuncs", "methods", "constants", "properties", "signals" })
infos.property = infos.base:derive { "typeinfo", "flags", "transfer" }
infos.callable = infos.base:derive({ "return_type", "return_transfer" }, { "args" })
...
infos.arg = infos.base:derive { "typeinfo", "direction", "transfer", "optional", "typeinfo" }
infos.struct = infos.registered:derive({ "is_gtype_struct", "size" }, { "fields", "methods" })
