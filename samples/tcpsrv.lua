...
local app = Gio.Application { application_id = "org.lgi.samples.tcptest", flags = Gio.ApplicationFlags.NON_UNIQUE }

local service = Gio.SocketService.new()
service:add_address(Gio.InetSocketAddress.new_from_string("127.0.0.1", 3333), Gio.SocketType.STREAM, Gio.SocketProtocol.TCP)
...
function service:on_incoming(conn)
	...
	print(string.format("Incoming connection from %s:%s", rc:get_address():to_string(), rc:get_port()))
