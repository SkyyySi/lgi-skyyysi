...
local function redisplay(rl, newpos, modified)
	if newpos < rl.pos then
		...
		if oldc ~= newc then
			rl.display(("\27[%d%s"):format(math.abs(newc - oldc), oldc < newc and "C" or "D"))
		end
...
function bindings.default(rl, key)
	if not key:match("%c") then
		rl.input = rl.input:sub(1, rl.pos - 1) .. key .. rl.input:sub(rl.pos + (rl.insert_mode and 0 or 1), -1)
