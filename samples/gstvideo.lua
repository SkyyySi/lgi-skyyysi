...
local colorspace = Gst.ElementFactory.make("videoconvert", "colorspace") or Gst.ElementFactory.make("ffmpegcolorspace", "colorspace")
...
local function bus_callback(bus, message)
	...
	if message.type.TAG then
		message:parse_tag():foreach(function(list, tag)
