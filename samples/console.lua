...
local function Console()
	...
	local actions = {
		...
		multiline = Gtk.ToggleAction {
			name = "multiline",
			stock_id = Gtk.STOCK_JUSTIFY_LEFT,
			is_important = true,
			label = "_Multiline",
		...
		up = Gtk.Action {
			...
			label = "_Previous",
			tooltip = "Previous command in history",
			sensitive = false,
		},
		...
		down = Gtk.Action {
			...
			label = "_Next",
			tooltip = "Next command in history",
			sensitive = false,
		},
					...
					Gtk.TextView {
						...
						buffer = Gtk.TextBuffer {
							tag_table = Gtk.TextTagTable {
								...
								Gtk.TextTag { name = "result", style = Pango.Style.ITALIC },
								Gtk.TextTag { name = "error", weight = Pango.Weight.BOLD, foreground = "red" },
							},
						},
					},
				},
	...
	for _, schema in pairs(Gio.Settings.list_schemas()) do
		if schema == "org.gnome.desktop.interface" then
			font_button.font_name = Gio.Settings({ schema = schema }):get_string("monospace-font-name")
			break
		end
	end
	...
	entry.buffer.on_notify["cursor-position"] = function(buffer)
		...
		indicator.label = iter:get_line() + 1 .. ":" .. iter:get_line_offset() + 1
	end
	...
	function actions.execute:on_activate()
		...
		if tag == "error" then
			...
			if line then
				entry.buffer:place_cursor(entry.buffer:get_iter_at_line_offset(line - 1, 0))
			end
		else
	...
	function entry:on_key_press_event(event)
		...
		if not action or state.SHIFT_MASK or actions.multiline.active == without_control then
			return false
		end

		-- Ask textview whether it still wants to consume the key.
		if self:im_context_filter_keypress(event) then
			return true
		end
