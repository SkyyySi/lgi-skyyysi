...
local app = Gio.Application { application_id = "org.lgi.samples.udptest", flags = Gio.ApplicationFlags.NON_UNIQUE }

local socket = Gio.Socket.new(Gio.SocketFamily.IPV4, Gio.SocketType.DATAGRAM, Gio.SocketProtocol.UDP)
...
source:set_callback(function()
	...
	if len > 0 then
		...
		print(string.format("%s:%d %s", src:get_address():to_string(), src:get_port(), data))
	else
