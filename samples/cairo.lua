...
function samples.gradient(cr)
	...
	pat = cairo.Pattern.create_radial(115.2, 102.4, 25.6, 102.4, 102.4, 128)
	pat:add_color_stop_rgba(0, 1, 1, 1, 1)
...
function samples.imagepattern(cr)
	...
	pattern.matrix = cairo.Matrix.create_scale(image.width / 256 * 5, image.height / 256 * 5)
	cr.source = pattern
...
function samples.rounded_rectangle(cr)
	...
	cr:new_sub_path()
	cr:arc(x + width - radius, y + radius, radius, math.rad(-90), math.rad(0))
	cr:arc(x + width - radius, y + height - radius, radius, math.rad(0), math.rad(90))
	cr:arc(x + radius, y + height - radius, radius, math.rad(90), math.rad(180))
	cr:arc(x + radius, y + radius, radius, math.rad(180), math.rad(270))
	cr:close_path()
