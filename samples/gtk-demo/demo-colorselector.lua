return function(parent, dir)
	...
	area:override_background_color(0, Gdk.RGBA { red = 0, green = 0, blue = 1, alpha = 1 })
	...
	function window.child.change:on_clicked()
		...
		if Gtk.ColorChooserDialog then
			...
			dialog.color_selection.current_rgba = self.style_context:get_background_color("NORMAL")
			function dialog:on_response(response_id)
				if response_id == Gtk.ResponseType.OK then
					area:override_background_color(0, self.color_selection.current_rgba)
				end
				dialog:hide()
			end
		end
