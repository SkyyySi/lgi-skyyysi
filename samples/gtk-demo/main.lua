...
sorted_model:set_sort_func(DemoListColumn.TITLE, function(model, a, b)
		...
		Gtk.ScrolledWindow {
			...
			Gtk.TreeView {
				...
				Gtk.TreeViewColumn {
					...
					{ Gtk.CellRendererText {}, { text = DemoListColumn.TITLE, style = DemoListColumn.STYLE } },
				},
			},
		},
		Gtk.Notebook {
...
window.child.source:override_font(Pango.FontDescription.from_string("monospace"))
...
local function append_text(buffer, text, tag)
	...
	if tag then
		...
		buffer:apply_tag_by_name(tag, buffer:get_iter_at_offset(offset), end_iter)
	end
end
...
function app:on_activate()
	...
	local pixbuf, err = GdkPixbuf.Pixbuf.new_from_file(dir:get_child("gtk-logo-rgb.gif"):get_path())
	if pixbuf then
