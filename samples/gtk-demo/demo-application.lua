return function(parent, dir)
	...
	local logo_pixbuf = GdkPixbuf.Pixbuf.new_from_file(dir:get_child("gtk-logo-rgb.gif"):get_path())
	...
	local function activate_action(action)
		local dialog = Gtk.MessageDialog {
			...
			message_type = "INFO",
			buttons = "CLOSE",
			text = ("You activated action: '%s' of type '%s'"):format(action.name, GObject.Type.name(action._type)),
	...
	local function change_radio_action(action)
		if action.active then
			message_label.label = (("You activated radio action: '%s' of type '%s'." .. "\nCurrent value: %d"):format(action.name, GObject.Type.name(action._type), action.value))
	...
	local function about_cb()
		local about = Gtk.AboutDialog {
			...
			version = ("Running against GTK+ %d.%d.%d, Lgi %s"):format(Gtk.get_major_version(), Gtk.get_minor_version(), Gtk.get_micro_version(), lgi._VERSION),
	...
	local action_group = Gtk.ActionGroup {
		...
		{ Gtk.Action { name = "New", label = "_New", stock_id = Gtk.STOCK_NEW, tooltip = "Create a new file", on_activate = activate_action }, accelerator = "<control>N" },
		...
		Gtk.Action { name = "File1", label = "File1", tooltip = "Open first file", on_activate = activate_action },
		{ Gtk.Action { name = "Save", label = "_Save", tooltip = "Save current file", stock_id = Gtk.STOCK_SAVE, on_activate = activate_action }, accelerator = "<control>S" },
		Gtk.Action { name = "SaveAs", label = "Save _As", tooltip = "Save to a file", stock_id = Gtk.STOCK_SAVE, on_activate = activate_action },
		{ Gtk.Action { name = "Quit", label = "_Quit", tooltip = "Quit", stock_id = Gtk.STOCK_QUIT, on_activate = activate_action }, accelerator = "<control>Q" },
		Gtk.Action { name = "About", label = "_About", tooltip = "About", on_activate = about_cb },
		Gtk.Action { name = "Logo", stock_id = "demo-gtk-logo", tooltip = "GTK+ on LGI", on_activate = activate_action },

		{ Gtk.ToggleAction { name = "Bold", stock_id = Gtk.STOCK_BOLD, label = "_Bold", tooltip = "Bold", on_activate = activate_action, active = true }, accelerator = "<control>B" },
		Gtk.ToggleAction { name = "DarkTheme", label = "_Prefer dark theme", tooltip = "Prefer dark theme", active = false, on_activate = set_theme },
		{
			{ Gtk.RadioAction { name = "Red", label = "_Red", tooltip = "Blood", value = color.RED }, accelerator = "<control>R" },
			{ Gtk.RadioAction { name = "Green", label = "_Green", tooltip = "Grass", value = color.GREEN }, accelerator = "<control>G" },
			{ Gtk.RadioAction { name = "Blue", label = "_Blue", tooltip = "Sky", value = color.BLUE }, accelerator = "<control>B" },
			on_change = change_radio_action,
		},

		{
			{ Gtk.RadioAction { name = "Square", label = "_Square", tooltip = "Square", value = shape.SQUARE }, accelerator = "<control>S" },
			{ Gtk.RadioAction { name = "Rectangle", label = "_Rectangle", tooltip = "Rectangle", value = shape.RECTANGLE }, accelerator = "<control>R" },
			{ Gtk.RadioAction { name = "Oval", label = "_Oval", tooltip = "Oval", value = shape.OVAL }, accelerator = "<control>O" },
			on_change = change_radio_action,
		},
	}
...
<ui>
  ...
  <toolbar name='ToolBar'>
	...
	local function update_statusbar()
		...
		local msg = statusbar:push(0, ("Cursor at row %d column %d - %d chars in document"):format(iter:get_line(), iter:get_line_offset(), buffer:get_char_count()))
	end
