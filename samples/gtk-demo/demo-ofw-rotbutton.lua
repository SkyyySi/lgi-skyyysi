return function(parent, dir)
	...
	function GtkDemo.RotatedBin:do_realize()
		...
		function window:on_pick_embedded_child(widget_x, widget_y)
			if bin.priv.child and bin.priv.child.visible then
				...
				if x >= 0 and x < child_area.width and y >= 0 and y < child_area.height then
	...
	function GtkDemo.RotatedBin:do_size_allocate(allocation)
		...
		if self.realized then
			self.window:move_resize(allocation.x + self.border_width, allocation.y + self.border_width, w, h)
		end
		...
		if self.priv.child and self.priv.child.visible then
			...
			elseif s == 0 then
				...
				child_allocation.width = math.min((w - s * child_allocation.height) / c, (h - c * child_allocation.width) / s)
			end
			if self.realized then
				self.priv.offscreen_window:move_resize(child_allocation.x, child_allocation.y, child_allocation.width, child_allocation.height)
			end
	...
	function GtkDemo.RotatedBin:_class_init()
		...
		GObject.signal_override_class_closure(GObject.signal_lookup("damage-event", Gtk.Widget), GtkDemo.RotatedBin, GObject.Closure(GtkDemo.RotatedBin.do_damage, Gtk.Widget.on_damage_event))
	end

	function GtkDemo.RotatedBin:do_draw(cr)
		if cr:should_draw_window(self.window) then
			if self.priv.child and self.priv.child.visible then
				local surface = Gdk.offscreen_window_get_surface(self.priv.offscreen_window)
				...
				-- clip
				cr:rectangle(0, 0, self.priv.offscreen_window:get_width(), self.priv.offscreen_window:get_height())
				cr:clip()
		...
		if cr:should_draw_window(self.priv.offscreen_window) then
			Gtk.render_background(self.style_context, cr, 0, 0, self.priv.offscreen_window:get_width(), self.priv.offscreen_window:get_height())
			if self.priv.child then
