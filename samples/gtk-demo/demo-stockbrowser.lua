return function(parent, dir)
	...
	local function create_model()
		...
		for _, id in ipairs(ids) do
			...
			local macro = GLib.ascii_strup(id, -1):gsub("^GTK%-", "Gtk.STOCK_"):gsub("%-", "_")
			...
			if icon_set then
				...
				if size ~= Gtk.IconSize.MENU then
					...
					small_icon = small_icon:scale_simple(icon_width, icon_height, "BILINEAR")
				end
			end
	...
	function selection:on_changed()
		...
		if model and iter then
			...
			if row[BrowserColumn.LABEL] then
				display.accel_label.label = ("%s %s"):format(row[BrowserColumn.LABEL], row[BrowserColumn.ACCEL_STR] or "")
			else
			...
			if row[BrowserColumn.SMALL_ICON] then
				...
				for _, size in ipairs(Gtk.IconFactory.lookup_default(row[BrowserColumn.ID]):get_sizes()) do
