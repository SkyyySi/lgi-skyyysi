return function(parent, dir)
	...
	local function create_stock_icon_store()
		...
		for _, stock in ipairs {
			...
			if stock ~= "" then
				store:append {
					[ComboColumn.PIXBUF] = cellview:render_icon_pixbuf(stock, Gtk.IconSize.BUTTON),
	...
	local function create_capital_store()
		...
		for _, group in ipairs {
			{ name = "A - B", "Albany", "Annapolis", "Atlanta", "Augusta", "Austin", "Baton Rouge", "Bismarck", "Boise", "Boston" },
			{ name = "C - D", "Carson City", "Charleston", "Cheyenne", "Columbia", "Columbus", "Concord", "Denver", "Des Moines", "Dover" },
			{ name = "E - J", "Frankfort", "Harrisburg", "Hartford", "Helena", "Honolulu", "Indianapolis", "Jackson", "Jefferson City", "Juneau" },
			{ name = "K - O", "Lansing", "Lincoln", "Little Rock", "Madison", "Montgomery", "Montpelier", "Nashville", "Oklahoma City", "Olympia" },
			{ name = "P - S", "Phoenix", "Pierre", "Providence", "Raleigh", "Richmond", "Sacramento", "Salem", "Salt Lake City", "Santa Fe", "Springfield", "St. Paul" },
			{ name = "T - Z", "Tallahassee", "Topeka", "Trenton" },
		} do
	...
	window.child.stringids:bind_property("active-id", window.child.ids_entry, "text", "BIDIRECTIONAL")
