return function(parent, dir)
	...
	local outdir = GLib.get_user_special_dir("DIRECTORY_DOCUMENTS") or GLib.get_home_dir()
	settings:set(Gtk.PRINT_OUTPUT_URI, "file://" .. outdir .. "/gtk-demo." .. (settings:get(Gtk.PRINT_OUTPUT_FILE_FORMAT) or "pdf"))
	...
	function operation:on_begin_print(context)
		...
		contents.filename = file:query_info(Gio.FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME, "NONE"):get_display_name()
		...
		while true do
			...
			if not line and len == 0 then
				break
			end
			assert(line, len)
			contents[#contents + 1] = line
		end
		contents.num_pages = math.floor((#contents - 1) / contents.lines_per_page + 1)
