return function(parent, dir)
	...
	window:get_content_area():add(Gtk.Box {
			...
			Gtk.Box {
				...
				Gtk.ScrolledWindow {
					...
					Gtk.TreeView {
						...
						Gtk.TreeViewColumn {
							title = "Name",
							{ Gtk.CellRendererText(), { text = DisplayColumn.NAME } },
						},
					},
				},
				Gtk.Box {
			...
			Gtk.Box {
				...
				Gtk.ScrolledWindow {
					...
					Gtk.TreeView {
						...
						Gtk.TreeViewColumn {
							title = "Number",
							{ Gtk.CellRendererText(), { text = ScreenColumn.NUMBER } },
						},
					},
				},
				Gtk.Box {
	...
	local function change_display()
		...
		local grab_status = device:grab(popup.window, "APPLICATION", false, "BUTTON_RELEASE_MASK", cursor, Gdk.CURRENT_TIME)
	...
	local function add_display(display)
		...
		local handler_id = display.on_closed:connect(function(is_error)
	...
	local handler_id = display_manager.on_display_opened:connect(function(display)
		add_display(display)
	end)
	function window:on_destroy()
