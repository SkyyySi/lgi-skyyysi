return function(parent, dir)
	...
	-- Load pixbuf images.
	local background = assert(GdkPixbuf.Pixbuf.new_from_file(dir:get_child("background.jpg"):get_path()))
	...
	for _, name in ipairs {
		...
		images[#images + 1] = assert(GdkPixbuf.Pixbuf.new_from_file(dir:get_child(name):get_path()))
	end
	...
	local frame_num = 0
	local sin, cos, pi, floor, abs, min, max = math.sin, math.cos, math.pi, math.floor, math.abs, math.min, math.max
	local timeout_id = GLib.timeout_add(GLib.PRIORITY_DEFAULT, FRAME_DELAY, function()
		...
		local r2 = Gdk.Rectangle { x = 0, y = 0, width = back_width, height = back_height }
		...
		for i = 1, #images do
			...
			if dest then
				...
				images[i]:composite(frame, dest.x, dest.y, dest.width, dest.height, xpos, ypos, k, k, "NEAREST", max(127, abs(alpha)))
			end
		end
