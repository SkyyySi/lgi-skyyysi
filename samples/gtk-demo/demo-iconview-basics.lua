return function(parent, dir)
	...
	local pixbuf = {
		REGULAR = assert(GdkPixbuf.Pixbuf.new_from_file(dir:get_child("gnome-fs-regular.png"):get_path())),
		DIRECTORY = assert(GdkPixbuf.Pixbuf.new_from_file(dir:get_child("gnome-fs-directory.png"):get_path())),
	}
	...
	store:set_default_sort_func(function(model, a, b)
		-- Sort folders before files.
		a = model[a]
		b = model[b]
		local is_dir_a, is_dir_b = a[ViewColumn.IS_DIRECTORY], b[ViewColumn.IS_DIRECTORY]
		if not is_dir_a and is_dir_b then
			return 1
		elseif is_dir_a and not is_dir_b then
			return -1
		else
			return GLib.utf8_collate(a[ViewColumn.DISPLAY_NAME], b[ViewColumn.DISPLAY_NAME])
		end
	end)
	store:set_sort_column_id(Gtk.TreeSortable.DEFAULT_SORT_COLUMN_ID, "ASCENDING")
