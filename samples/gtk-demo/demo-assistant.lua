return function(parent, dir)
	...
	local assistant = Gtk.Assistant {
		...
		},
		{
			...
			Gtk.Grid {
				...
				Gtk.CheckButton { label = "This is optional data, you may continue " .. "even if you do not check this" },
			},
		},
		{
			title = "Confirmation",
			type = "CONFIRM",
			complete = true,
			Gtk.Label { label = "This is a confirmation page, press 'Apply' " .. "to apply changes" },
		},
		{
			title = "Applying changes",
			type = "PROGRESS",
			Gtk.ProgressBar { id = "progressbar", halign = "CENTER", valign = "CENTER" },
		},
	}
	...
	function assistant:on_prepare()
		self.title = ("Sample assistant (%d of %d)"):format(self:get_current_page() + 1, self:get_n_pages())
