return function(parent, dir)
	...
	for i = 1, 2 do
		...
		if i == 1 then
			...
			local count = 0
			timer = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 50, function()
				-- Insert to the 'end' mark.
				buffer:insert(buffer:get_iter_at_mark(mark), "\n" .. (" "):rep(count) .. "Scroll to end scroll to end scroll to end " .. "scroll to end ", -1)
			...
			timer = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 100, function()
				-- Insert some text into the buffer.
				local iter = buffer:get_end_iter()
				buffer:insert(iter, "\n" .. (" "):rep(count) .. "Scroll to bottom scroll to bottom scroll to bottom " .. "scroll to bottom ", -1)
