return function(parent, dir)
	...
	window.child.circle:override_background_color("NORMAL", Gdk.RGBA { red = 1, green = 1, blue = 1, alpha = 1 })

	local function fancy_shape_renderer(cr, attr, do_path)
		cr:translate(cr:get_current_point())
		cr:scale(attr.ink_rect.width / Pango.SCALE, attr.ink_rect.height / Pango.SCALE)
	...
	local function create_fancy_attr_list_for_layout(layout)
		...
		local rect = Pango.Rectangle { x = 0, width = ascent, y = -ascent, height = ascent }
	...
	function window.child.circle:on_draw(cr)
		...
		cr:translate(device_radius + (self.width - 2 * device_radius) / 2, device_radius + (self.height - 2 * device_radius) / 2)
		cr:scale(device_radius / RADIUS, device_radius / RADIUS)

		-- Create a subtle gradient source and use it.
		local pattern = cairo.Pattern.create_linear(-RADIUS, -RADIUS, RADIUS, RADIUS)
