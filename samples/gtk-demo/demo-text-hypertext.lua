...
function(text)
	...
	function textview:on_motion_notify_event(event)
		set_cursor_if_appropriate(self, self:window_to_buffer_coords("WIDGET", event.x, event.y))
		return false
	end
	...
	function textview:on_visibility_notify_event(event)
		...
		if x and y then
			set_cursor_if_appropriate(self, self:window_to_buffer_coords("WIDGET", x, y))
		end
		return false
	end
	...
	function textview:on_event_after(event)
		if event.type == "BUTTON_RELEASE" and event.button.button == 1 then
			...
			if not start or not stop or start:get_offset() == stop:get_offset() then
				follow_if_link(self, self:get_iter_at_location(self:window_to_buffer_coords("WIDGET", event.button.x, event.button.y)))
			end
		end
	end
	...
	function textview:on_key_press_event(event)
		if event.keyval == Gdk.KEY_Return or event.keyval == Gdk.KEY_KP_Enter then
			follow_if_link(self, self.buffer:get_iter_at_mark(self.buffer:get_insert()))
		end
		return false
	end
