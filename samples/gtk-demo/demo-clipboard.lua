return function(parent, dir)
	...
	local function get_image_pixbuf(image)
		...
		elseif image.storage_type == "STOCK" then
			...
			log.warning(("Image storage type \"%s\" not handled"):format(image.storage_type))
		end
	end

	local function drag_begin(ebox, context)
		Gtk.drag_set_icon_pixbuf(context, get_image_pixbuf(ebox:get_child()), -2, -2)
	end
	...
	local window = Gtk.Window {
		...
		Gtk.Box {
			...
			Gtk.Label { label = "\"Paste\" will paste the text from " .. "the clipboard to the entry" },
			Gtk.Box {
				orientation = "HORIZONTAL",
				spacing = 4,
				border_width = 8,
			...
			Gtk.Box {
				...
				Gtk.EventBox {
					...
					Gtk.Image { stock = Gtk.STOCK_DIALOG_WARNING, icon_size = Gtk.IconSize.BUTTON },
				},
				Gtk.EventBox {
					id = "ebox2",
					on_drag_begin = drag_begin,
					...
					on_drag_data_received = drag_data_received,
					on_button_press_event = button_press,
					Gtk.Image { stock = Gtk.STOCK_STOP, icon_size = Gtk.IconSize.BUTTON },
				},
			},
		},
	}
